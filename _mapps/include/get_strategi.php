...
if(empty($T)){
	...
	if($_SESSION['SESS_ULEVEL']==3){	
		...
		if(!empty($_SESSION['SESS_AGENSIGRP'])){ $sqlP .= " AND C.agensia_id=".tosql($_SESSION['SESS_AGENSIGRP']); }
		if(!empty($_SESSION['SESS_AGENSIID'])){ $sqlP .= " AND B.agensi_id=".tosql($_SESSION['SESS_AGENSIID']); }
	} else {
