...
?>
	...
	<div class="form-group">
		<div class="row">
			...
			<div class="col-sm-3">
				<select name="stp_tahun_2" id="stp_tahun_2" class="form-control" onchange="set_stpm('stp_tahun_2',this.value,'R')">
					<option value="">Sila pilih tahun</option>
					<?php for($t=$tahun_1;$t<=$tahun_2;$t++){
						print '<option value="'.$t.'"'; 
						if($data['stp_tahun_2']==$t){ print 'selected'; }
						print '>'.$t.'</option>';
					} ?>
				</select>
			</div>
			...
			<div class="col-sm-3">
				<select class="form-control" id="stp_jenis_2" name="stp_jenis_2" onchange="set_stpm('stp_jenis_2',this.value,'')">
					<!-- <option value="">Sila pilih jenis sijil</option> -->
					<?php while(!$rssijil->EOF){ ?>
					<option value="<?=$rssijil->fields['KOD'];?>" 
					<?php if($rssijil->fields['KOD']==$data['stp_jenis_2']){ print 'selected';} ?>	
	...
	<?php if(!empty($data['stp_tahun_2'])){ 
		$rsSRP = $conn->query("SELECT * FROM $schema1.`ref_matapelajaran` WHERE `TKT`='6' AND `SAH_YT`='Y' AND `GAB_YT`='T' AND `kod` NOT IN ('900') 
			ORDER BY `DISKRIPSI`");
		$rsGred = $conn->query("SELECT * FROM $schema1.`ref_gred_matapelajaran` WHERE `TKT`='6' AND `JENIS`='D' 
			ORDER BY `SUSUNAN`");
	...
	<div class="form-group">
		<div class="row">
			<div class="col-sm-8">
				<div class="row">
					...
					<div class="col-sm-4 col-xm-4" style="padding-bottom:5px">
						...
						<select class="form-control" name="gred_1[]" id="gred_10">
							...
							<?php //$conn->debug=true;
							$result = get_stp_result($conn, $uid, "900", $tahun2, 'BT');
							// $grd = dlookup("$schema2.`calon_stp_stam`","gred","`id_pemohon`='{$uid}' AND `tahun`='{$tahun}' AND `matapelajaran`='A33' AND `jenis_xm`='BT'");
							$datas='900';
							$rsGred->movefirst();
							while(!$rsGred->EOF){ ?>
				...
				$rsResult = $conn->query("SELECT * FROM $schema2.`calon_stp_stam` WHERE `jenis_xm`='BT' AND `id_pemohon`=".tosql($uid). " AND tahun=".tosql($tahun2). " AND matapelajaran NOT IN ('900')");
				while(!$rsResult->EOF){ 
		...
		<div class="modal-footer" style="padding:0px;">
			...
			<label class="btn btn-danger" onclick="do_hapus_url('akademik/sql_akademik.php?frm=STPM&pro=STPM2_DEL&id_pemohon=<?=$uid;?>')">Hapus Maklumat</label>
