...
function save_univ(){
	...
	var tarikh_limit = formatDateLimit(d_limit);
	...
	if(tkh_senate.trim() !=''){
		if(tkh > tarikh_limit){
        		msg = msg+'\n- Sila masukkan tarikh senat sebelum 31/12/2025.';
        		$("#tkh_senate").css("border-color","#f00");
		}
    } 
...
function hidemm(){
	...
	if(peringkat1=='3' || peringkat1=='4'){
		...
		$(major1).append("<option value='' selected></option>");
		$(minor1).append("<option value='' selected></option>");
		$('#selmajor1').hide();
		$('#selminor1').hide();
	}
}
...
function get_tahun(){
	var jenisMuetCefr1 = $('#jenisMuetCefr1').val();
	var sel="#tahunMuetCefr1";
	$.ajax({
		url: '../include/get_tahunCfer.php?jenisMuetCefr1='+jenisMuetCefr1 ,
		type: 'post',
		data: {jenisMuetCefr1:jenisMuetCefr1},
		dataType: 'json',
		success:function(response){
...
<div class="form-group">
	<div class="row">
		<div class="col-sm-8"><br>
				...
				<div class="row">
					...
					<div class="col-sm-4">
						<select class="form-control" name="tahun1" id="tahun1">
							...
							<option value="<?=$t1;?>" <?php if($rsUniv->fields['tahun']==$t1){ print 'selected'; }?>><?=$t1;?></option>	
							<?php } ?>
						</select>
					</div>
				</div>
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-4">
					<!--<input type="date" id="tkh_senate" name="tkh_senate" class="form-control" value="<?php print $rsUniv->fields['tkh_senate'];?>">-->
					<!--<input type="text" name="tkh_senate" id="tkh_senate"  size="15" maxlength="10" value="<?php print DisplayDate($rsUniv->fields['tkh_senate']);?>" 
						data-inputmask-alias="datetime" data-inputmask-inputformat="dd/mm/yyyy" data-mask 
						class="form-control disableFuturedate icon" style="background-color: #fff; cursor: pointer;">-->

						<input type="text" id="tkh_senate" name="tkh_senate"  size="15" maxlength="10" value="<?php print DisplayDate($rsUniv->fields['tkh_senate']);?>"
						data-inputmask-alias="datetime" data-inputmask-inputformat="dd/mm/yyyy" data-mask 
						class="form-control enableFuturedate icon" style="background-color: #fff; cursor: pointer;">	
					</div>
				</div>
			</div>
			<?php 
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="peringkat1" id="peringkat1" class="form-control" onchange="get_univ(this.value)">
							...
							<option value="<?=$rfKelulusan->fields['kod'];?>" <?php if($rsUniv->fields['peringkat']==$rfKelulusan->fields['kod']){ print 'selected'; }?>><?php print $rfKelulusan->fields['diskripsi'];?></option>	
			...
			<?php
			$cgpa = $rsUniv->fields['cgpa'];
			if(empty($cgpa)){ $cgpa=''; }
			?>
			<div class="form-group">
				<div class="row">
					<label for="nama" class="col-sm-4 control-label"><b>CGPA (PNGK) <div style="float:right"><div style="float:right">
							<a title="Masukkan 0.00 jika tiada CGPA/ tidak berkenaan. Pemohon tidak dibenarkan untuk membundarkan CGPA(PNGK)." >
                    						<i class="fa fa-info-circle fa-lg" aria-hidden="true"></i>
                					</a> :
						</div>
					</div></b></label>
			...
			<?php 
				...
				// WHERE A.`KOD`=B.`KOD` AND A.`sijil_kod`=".tosql($rsUniv->fields['peringkat'])." ORDER BY B.`DISKRIPSI`";
				// $query ="SELECT B.`KOD`, B.`DISKRIPSI` FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B  
				// WHERE A.`kod_institusi`=B.`KOD` AND A.`is_deleted`=0 AND A.`status`=0 AND A.`kod_peringkat_kelulusan`=".tosql($rsUniv->fields['peringkat']);  
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="inst_keluar_sijil1" id="inst_keluar_sijil1" class="form-control select2 inst_keluar_sijil1" style="width: 100%;" aria-hidden="true" onchange="get_pengkhususan(this.value)">
							...
							<option value="<?=$rsInstitusi->fields['KOD'];?>" <?php if($rsUniv->fields['inst_keluar_sijil']==$rsInstitusi->fields['KOD']){ print 'selected'; }?>><?php print $rsInstitusi->fields['DISKRIPSI'];?></option>	
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="pengkhususan1" id="pengkhususan1" class="form-control select2 pengkhususan1" style="width: 100%;" aria-hidden="true">
							...
							<option value="<?=$rsKhusus->fields['kod'];?>" <?php if($rsUniv->fields['pengkhususan']==$rsKhusus->fields['kod']){ print 'selected'; }?>><?php print $rsKhusus->fields['DISKRIPSI'];?></option>	
			...
			<?php 
				$query ="SELECT KOD, DISKRIPSI FROM $schema1.ref_major
				WHERE STATUS=0
				ORDER BY DISKRIPSI ASC";
				$qrymajor = $conn->query($query);
			?>
			<div class="form-group" id="selmajor1" name="selmajor1">
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="major1" id="major1" class="form-control select2" style="width: 100%;" aria-hidden="true">
							<option value="" selected>Sila Pilih</option>
							<?php while(!$qrymajor->EOF){ ?>
							<option value="<?=$qrymajor->fields['KOD'];?>" <?php if($rsUniv->fields['MAJOR']==$qrymajor->fields['KOD']){ print 'selected'; }?>>
			...
			<?php 
				$query ="SELECT KOD, DISKRIPSI FROM $schema1.ref_minor
				WHERE STATUS=0
				ORDER BY DISKRIPSI ASC";
			...
			<div class="form-group" id="selminor1" name="selminor1">
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="minor1" id="minor1" class="form-control select2" style="width: 100%;" aria-hidden="true">
							<option value="">Sila pilih </option>
							<?php while(!$qryminor->EOF){ ?>
							<option value="<?=$qryminor->fields['KOD'];?>" <?php if($rsUniv->fields['MINOR']==$qryminor->fields['KOD']){ print 'selected'; }?>>
					...
					<div class="col-sm-4">
						...
						<select name="tahunMuetCefr1" id="tahunMuetCefr1" class="form-control" onchange="get_keputusan(this.value)">
							...
							<?php for($i=$d; $i>='1999'; $i--){ ?>
                   			<option value="<?=$i;?>" <?php if($rsUniv->fields['muet_tahun']==$i){ print 'selected'; }?>><?=$i;?></option>
                    		<?php } ?>						
						</select>
					</div>
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="biasiswa1" id="biasiswa" class="form-control">
							...
							<option value="<?=$rsBiasiswa->fields['kod_biasiswa'];?>" <?php if($rsUniv->fields['biasiswa']==$rsBiasiswa->fields['kod_biasiswa']){ print 'selected'; }?>><?php print $rsBiasiswa->fields['biasiswa'];?></option>	
		...
		$rsSijil3 = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='UNIV1B' AND `id_pemohon`=".tosql($uid));
		//if(empty($rsSijil3->fields['sijil_nama'])){ $sijil2 ="../upload_doc/sijil_muet.jpg"; }
		//else { $sijil2 = "/upload/".$uid."/".$rsSijil3->fields['sijil_nama']; }
		if(empty($rsSijil->fields['sijil_nama'])){ $sijil_pic2 ="/var/www/html/myspp/upload_doc/sijil_muet.jpg"; }
		else { $sijil_pic2 = "/var/www/upload/".$uid."/".$rsSijil3->fields['sijil_nama']; }
 		//print $sijil;
		...
		<div class="col-sm-4" align="center" style="border: 2px solid black; padding: 10px; border-radius: 25px;">
			...
			<?php 
			if(empty($rsSijil31->fields['sijil_nama'])){ 
				print "Tiada Sijil Dimuat Naik"; 
			} else { 
				$sijil_pic3 = "/var/www/upload/".$uid."/".$rsSijil31->fields['sijil_nama'];
				if (file_exists($sijil_pic3)){
			?>
				<!--<a href="../view_doc_pemohon.php?id_pemohon=<?=$uid;?>&doc=<?=$rsSijil31->fields['sijil_nama'];?>" data-toggle="modal" 
					data-target="#myModal" data-backdrop="">.</a>-->
		...
		<div class="col-sm-4" align="center" style="border: 2px solid black; padding: 10px; border-radius: 25px;">
			...
			<?php print $rsSijil3->fields['sijil_nama'];?>
