...
function save_univ(){
	...
	$.each(other_data,function(key,input){
        ...
        $.ajax({
	        url:'akademik/sql_akademik.php?frm=UNIV&pro=SAVE3',
			type:'POST',
	        //dataType: 'json',
	        beforeSend: function () {
...
<div class="form-group">
	<div class="row">
		<div class="col-sm-8"><br>
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-4">
						<!--<input type="date" id="tkh_senate" name="tkh_senate" class="form-control" value="<?php print $rsUniv3->fields['tkh_senate'];?>">-->
						...
						<input type="text" name="tkh_senate" id="tkh_senate"  size="15" maxlength="10" value="<?php print DisplayDate($rsUniv3->fields['tkh_senate']);?>" 
						data-inputmask-alias="datetime" data-inputmask-inputformat="dd/mm/yyyy" data-mask 
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="peringkat1" id="peringkat1" class="form-control" onchange="get_univ(this.value)">
							...
							<option value="<?=$rfKelulusan->fields['kod'];?>" <?php if($rsUniv3->fields['peringkat']==$rfKelulusan->fields['kod']){ print 'selected'; }?>><?php print $rfKelulusan->fields['diskripsi'];?></option>	
			...
			<?php 
				...
				// WHERE A.`KOD`=B.`KOD` AND A.`sijil_kod`=".tosql($rsUniv3->fields['peringkat'])." ORDER BY B.`DISKRIPSI`";
				// $query ="SELECT B.`KOD`, B.`DISKRIPSI` FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B  
				// WHERE A.`kod_institusi`=B.`KOD` AND A.`is_deleted`=0 AND A.`status`=0 AND A.`kod_peringkat_kelulusan`=".tosql($rsUniv3->fields['peringkat']);  
			...
			<?php if(empty($rsUniv3->fields['inst_francais'])){ $inst_francais='T'; } else { $inst_francais = $rsUniv3->fields['inst_francais']; } ?>
			<div class="form-group">
				<div class="row">
			...
			<!-- <?php if(empty($rsUniv3->fields['bidang'])){ $bidang='S'; } else { $bidang = $rsUniv3->fields['bidang']; } ?>
			<div class="form-group">
				<div class="row">
			...
			<?php 
				//$conn->debug=true;
				$peringkats = $rsUniv3->fields['peringkat'];
				$query ="SELECT A.`kod`, B.`DISKRIPSI` FROM $schema1.`padanan_institusi_pengkhususan` A, $schema1.`ref_pengkhususan` B 
				WHERE A.`kod_pengkhususan`=B.`kod` AND A.`kod_institusi`=".tosql($rsUniv3->fields['inst_keluar_sijil'])." 
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="major1" id="major1" class="form-control select2" style="width: 100%;" aria-hidden="true">
							...
							<option value="<?=$qrymajor->fields['KOD'];?>" <?php if($rsUniv3->fields['MAJOR']==$qrymajor->fields['KOD']){ print 'selected'; }?>>
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="minor1" id="minor1" class="form-control select2" style="width: 100%;" aria-hidden="true">
							...
							<option value="<?=$qryminor->fields['KOD'];?>" <?php if($rsUniv3->fields['MINOR']==$qryminor->fields['KOD']){ print 'selected'; }?>>
				...
				<div class="row">
					...
					<div class="col-sm-8">
						<select name="jenisMuetCefr1" id="jenisMuetCefr1" class="form-control" onchange="get_keputusan(this.value)">
            				...
            				<option value="<?=$code;?>" <?php if($rsUniv3->fields['muet']==$code){ print 'selected'; }?>><?php print $rsJP->fields['diskripsi'];?></option>
        					<?php $rsJP->movenext(); } ?>
						</select>
					</div>
				</div>
					...
					<div class="col-sm-4">
						...
						<select name="tahunMuetCefr1" id="tahunMuetCefr1" class="form-control" onchange="get_keputusan(this.value)">
							...
							<?php for($t1=date("Y");$t1>=1999;$t1--){ ?>
                   			<option value="<?=$t1;?>" <?php if($rsUniv3->fields['muet_tahun']==$t1){ print 'selected'; }?>><?=$t1;?></option>
                    		<?php } ?>						
						</select>
					</div>
			...
			<div class="form-group">
				<div class="row">
					...
					<?php
					$sql3 = "SELECT * FROM $schema1.padanan_jenisPeperiksaan_keputusan WHERE kod_jenis_peperiksaan=".tosql($rsUniv3->fields['muet']);
					...
					<div class="col-sm-7">
						<select name="keputusanMuetCefr1" id="keputusanMuetCefr1" class="form-control">
							...
							?>    
            				<option value="<?=$code;?>" <?php if($rsUniv3->fields['muet_gred']==$code){ print 'selected'; }?>><?php print $kep;?></option>
        					<?php $rsJK->movenext(); } ?>
						</select>
					</div>
		...
		if(empty($rsSijil->fields['sijil_nama'])){ $sijil_pic1 ="/var/www/html/myspp/upload_doc/sijil_pmr.jpg"; }
		...
		$rsSijil2 = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='UNIV3B' AND `id_pemohon`=".tosql($uid));
		//if(empty($rsSijil2->fields['sijil_nama'])){ $sijil2="../upload_doc/sijil_muet.jpg"; }
		//else { $sijil2 = "/upload/".$uid."/".$rsSijil2->fields['sijil_nama']; }
		if(empty($rsSijil2->fields['sijil_nama'])){ $sijil_pic2 ="/var/www/html/myspp/upload_doc/sijil_pmr.jpg"; }
		...
		$rsSijil33 = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='UNIV3C' AND `id_pemohon`=".tosql($uid));
		...
		<div class="col-sm-4" align="center" style="border: 2px solid black; padding: 10px; border-radius: 25px;">
			...
			<?php if(empty($rsSijil33->fields['sijil_nama'])){ 
				print "Tiada Sijil Dimuat Naik"; 
			} else { 
				$sijil_pic3 = "/var/www/upload/".$uid."/".$rsSijil33->fields['sijil_nama'];
				if (file_exists($sijil_pic3)){
			?>
				<!--<a href="../view_doc_pemohon.php?id_pemohon=<?=$uid;?>&doc=<?=$rsSijil33->fields['sijil_nama'];?>" data-toggle="modal" data-target="#myModal"><?=$rsSijil33->fields['sijil_nama'];?></a>--> 
				<a href="../view_doc_pemohon.php?id_pemohon=<?=$uid;?>&doc=<?=$rsSijil33->fields['sijil_nama'];?>" target="_blank" 
				onclick="return popitup('../view_doc_pemohon.php?id_pemohon=<?=$uid;?>&doc=<?=$rsSijil33->fields['sijil_nama'];?>')"><?=$rsSijil33->fields['sijil_nama'];?></a> 
