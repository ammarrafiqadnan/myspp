...
function get_skim($conn, $kods){
	global $schema1;
	//$conn->debug=true;
	$sql = "SELECT KOD, DISKRIPSI FROM $schema1.`ref_skim` WHERE `SAH_YT`='Y' AND GGH_KOD IS NOT NULL ORDER BY KOD";
	$rs = $conn->query($sql);
	$data='';
	while(!$rs->EOF){
		$data .= '<option value="'.$rs->fields['KOD'].'"';
...
function get_gred_jawatan($conn, $kods){
	global $schema1;
	// $conn->debug=true;
	$sql = "SELECT KOD FROM $schema1.`ref_gred_gaji` WHERE `SAH_YT`='Y' AND KOD IS NOT NULL ORDER BY KOD";
	$rs = $conn->query($sql);
	$data='';
	while(!$rs->EOF){
		$data .= '<option value="'.$rs->fields['KOD'].'"';
