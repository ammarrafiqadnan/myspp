...
function do_sukan(){
	...
	var persatuan1 = $('#persatuan1').val().toUpperCase();
	var jawatan1 = $('#jawatan1').val();
    var pperingkat1 = $('#pperingkat1').val();
	var persatuan2 = $('#persatuan2').val().toUpperCase();
	var jawatan2 = $('#jawatan2').val();
    var pperingkat2 = $('#pperingkat2').val();
	var persatuan3 = $('#persatuan3').val().toUpperCase();
    ...
    if(persatuan1!='' && persatuan2!=''){
    	var item1 = persatuan1+jawatan1+pperingkat1;
    	var item2 = persatuan2+jawatan2+pperingkat2;
    	// alert(item1+":"+item2);
	    if(item1==item2){
    ...
    if(persatuan2!='' && persatuan3!=''){
    	var item1 = persatuan3+jawatan3+pperingkat3;
    	var item2 = persatuan2+jawatan2+pperingkat2;
    	// alert(item1+":"+item2);
	    if(item1==item2){
    ...
    if(persatuan1!='' && persatuan3!=''){
    	var item1 = persatuan1+jawatan1+pperingkat1;
    	var item2 = persatuan3+jawatan3+pperingkat3;
    	// alert(item1+":"+item2);
	    if(item1==item2){
	        msg = msg+'\n- Maklumat berkaitan persatuan bagi pilihan ketiga sama\ndengan pilihan pertama. Sila kemaskini.';
	        $("#persatua3").css("border-color","#f00");
				...
				<div class="row">
					<div class="col-sm-4 col-xs-5">
						<select name="sukan<?=$p;?>" id="sukan<?=$p;?>" class="form-control" onchange="on_schg(<?=$p;?>)">
							<option value="">Sila pilih maklumat sukan</option>
							<?php while(!$rsSukan->EOF){ ?>
							<option value="<?=$rsSukan->fields['kod'];?>"><?=$rsSukan->fields['deskripsi'];?></option>
							<?php $rsSukan->movenext(); } ?>
						</select>
					</div>
					<div class="col-sm-4 col-xs-5">
					...
					<div class="row">
						...
						<div class="col-sm-1">
							<img src="../images/trash.png" title="Hapus maklumat persatuan/kepimpinan" style="cursor: pointer;" 
								...
								<input type="hidden" name="pid_<?=$bil;?>" id="pid_<?=$bil;?>" value="1">
						</div>
					</div>
				</div>
				<?php $rsData->movenext(); } ?>
				...
				<div class="form-group">
					<div class="row">
						<div class="col-sm-5 col-xs-5">
							...
							class="form-control text-uppercase" onchange="on_chg(<?=$p;?>)" maxlength="60">
						</div>
						<div class="col-sm-3 col-xs-3">
							<select name="jawatan<?=$p;?>" id="jawatan<?=$p;?>" class="form-control" onchange="on_chg(<?=$p;?>)">
								<option value="">Sila pilih maklumat jawatan</option>
								<?php while(!$rsJawatan->EOF){ ?>
								<option value="<?=$rsJawatan->fields['KOD'];?>"><?=$rsJawatan->fields['DISKRIPSI'];?></option>
								<?php $rsJawatan->movenext(); } ?>
							</select>
						</div>
						<div class="col-sm-3 col-xs-3">
				...
				<div class="form-group">
					<div class="row">
						<div class="col-sm-12 col-xs-12">
							Adakah anda pernah menjalani Program Latihan Khidmat Negara? <font color="red">*</font> 
