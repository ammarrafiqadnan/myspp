							...
							<div class="col-sm-12">
								<table width="100%" cellspacing="1" cellpadding="10" border="1" align="center" class="table">
									...
									if($SPM_lengkap=='Lengkap'){ 
										if(!empty($data_spm['spm_jenis_sijil_1']) && $data_spm['spm_jenis_sijil_1']==5){
										...
										} else if(!empty($data_spm['spm_jenis_sijil_1']) && $data_spm['spm_jenis_sijil_1']<>5){
										...
										if(!empty($data_spm['spm_jenis_sijil_2']) && $data_spm['spm_jenis_sijil_2']==5){
										...
										} else if(!empty($data_spm['spm_jenis_sijil_2']) && $data_spm['spm_jenis_sijil_2']<>5){
									...
									if($SPM_lengkap=='Lengkap'){ 
										...
										if(!empty($data_spm['stam_tahun_2'])){ 
									...
									<?php
									$rsUniv1 = $conn->query("SELECT * FROM $schema2.`calon_ipt` WHERE bil_keputusan=1 AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
									$rsUniv2 = $conn->query("SELECT * FROM $schema2.`calon_ipt` WHERE bil_keputusan=2 AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
									$rsUniv3 = $conn->query("SELECT * FROM $schema2.`calon_ipt` WHERE bil_keputusan=3 AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));

									$bilUniv1 = $rsUniv1->recordcount();
									$bilUniv2 = $rsUniv2->recordcount();
									$bilUniv3 = $rsUniv3->recordcount();
									print 'aaaaaaaaaaaaaa'.$bilUniv1.':'.$bilUniv2.':'.$bilUniv3;
									...
									if($rsUniv->EOF){ 
										...
										$SPM_lengkap_ikon=$icon_info;
												...
												if(!empty($bilUniv3) && empty($rsUniv3->fields['tahun']) && empty($rsUniv3->fields['peringkat']) && empty($rsUniv3->fields['inst_keluar_sijil'])){ 
									 				$SPM_lengkap='<font style="color:red">Tidak Lengkap. Sila semak semula maklumat diisi.</font>'; 
										 			$SPM_lengkap_ikon=$icon_no;
										 		} else if(!empty($bilUniv2) && empty($rsUniv2->fields['tahun']) && empty($rsUniv2->fields['peringkat']) && empty($rsUniv2->fields['inst_keluar_sijil'])){ 
									 				$SPM_lengkap='<font style="color:red">Tidak Lengkap. Sila semak semula maklumat diisi.</font>'; 
										 			$SPM_lengkap_ikon=$icon_no;
										 		}  else if(!empty($bilUniv1) && empty($rsUniv1->fields['tahun']) && empty($rsUniv1->fields['peringkat']) && empty($rsUniv1->fields['inst_keluar_sijil'])){ 
									 				$SPM_lengkap='<font style="color:red">Tidak Lengkap. Sila semak semula maklumat diisi.</font>'; 
										 			$SPM_lengkap_ikon=$icon_no;
										 		} else {
													$SPM_lengkap='Lengkap';
									...
									if($SPM_lengkap=='Lengkap'){ 
											//$conn->debug=true;
											$rsSijil1A = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV1A') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											//$rsSijil1B = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV1B') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											$rsSijil1C = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV1C') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));

											$rsSijil2A = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV2A') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											//$rsSijil2B = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV2B') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											$rsSijil2C = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV2C') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));


											$rsSijil3A = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV3A') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											//$rsSijil3B = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV3B') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));
											$rsSijil3C = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil` IN ('UNIV3C') AND `id_pemohon`=".tosql($_SESSION['SESS_UID']));

											
												if(!empty($bilUniv3)){
													if(!empty($rsSijil3A->fields['jenis_sijil']) && !empty($rsSijil3C->fields['jenis_sijil'])){
														if(!empty($bilUniv2) && !empty($rsSijil2A->fields['jenis_sijil']) && !empty($rsSijil2C->fields['jenis_sijil'])){
															if(!empty($bilUniv1) && !empty($rsSijil1A->fields['jenis_sijil']) && !empty($rsSijil1C->fields['jenis_sijil'])){ 
																$SPM_lengkap='Lengkap'; 
																$SPM_lengkap_ikon=$icon_tick; 
																$btn_ok='';
															} else {
												...
												} else if(!empty($bilUniv2)){
													if(!empty($rsSijil2A->fields['jenis_sijil']) && !empty($rsSijil2C->fields['jenis_sijil'])){
														if(!empty($bilUniv1) && !empty($rsSijil1A->fields['jenis_sijil']) && !empty($rsSijil1C->fields['jenis_sijil'])){
															$SPM_lengkap='Lengkap'; 
															$SPM_lengkap_ikon=$icon_tick; 
															$btn_ok='';
												...
												} else if(!empty($bilUniv1)){ 
													if(!empty($rsSijil1A->fields['jenis_sijil']) && !empty($rsSijil1C->fields['jenis_sijil'])){
														$SPM_lengkap='Lengkap'; 
														$SPM_lengkap_ikon=$icon_tick; 
														$btn_ok='';
													} else {
			...
			<div class="box" style="background-color:#F2F2F2">
      			<div class="box-body">
					...
					<div class="modal-footer" style="padding:0px;">
						...
						<?php } else { ?>
							<font color="red">Terdapat maklumat yang tidak lengkap. Sila lengkapkan sebelum butang hantar diaktifkan. -></font>
