...
function Geeks(vals,bil) {
	// alert(bil);
	var data = document.myspp.chk.value;
    var nameArr = data.split(',');
    var k=''; var c='';
    var selectobject = document.getElementById("mp_1"+bil);
    for (var i = 0; i < nameArr.length; i++) {
        ...
        if(a==vals){ 
			swal({
	    		title: 'Amaran',
	    		text: 'Maklumat matapelajaran telah diada dalam senarai pilihan.',
	    		type: 'warning',
	    		confirmButtonClass: "btn-warning",
	    		confirmButtonText: "Ok",
	    		showConfirmButton: true,
		...
		<input type="hidden" name="actions" id="actions" value="<?php print $actions;?>" readonly="readonly"/>
			...
			<div class="col-md-12">
				...
				<div class="form-group">
					...
					$act2='';
					if($data['spm_jenis_sijil_1']==1 || $data['spm_jenis_sijil_1']==1){
						$act2 = "1,2";
	...
	//print_r($data);
                        ...
                        $s_sijil='';
               			if($actions==1){
							...
							$s_sijil = $data['spm_jenis_sijil_2'];
							$s_tahun = $data['spm_tahun_2'];
...
//print "<br>ACT:".$actions.":".$s_sijil.":".$s_tahun.":".$s_pangkat;
						
                        if($actions==1){
                        	if($data['spm_jenis_sijil_1']==5){
	                        	include 'akademik/svm1.php';
                        	} else {
	                        	include 'akademik/spm1.php';
                        	}
                        } else if($actions=='2'){
                        	if($data['spm_jenis_sijil_2']==5){
	                        	include 'akademik/svm1.php';
                        	} else {
	                            include 'akademik/spm1.php';
							}
