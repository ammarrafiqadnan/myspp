...
//print '<br><font color="white">'.$cond."</font>";
	...
	if($gcond=='8'){ $gcond=''; }
	else if($gcond=='1,8'){ $gcond='8'; } //updated
	else if($gcond=='1,2,8'){ $gcond='2,8'; } //updated
	else if($gcond=='1,2,9'){ $gcond='2,9'; } //new
	else if($gcond=='2,8'){ $gcond='2,8'; } //updated
	else if($gcond=='1,3,8'){ $gcond='3,8'; } //updated
	else if($gcond=='3,8'){ $gcond='3,8'; } //updated
	else if($gcond=='2,9'){ $gcond='2,9'; } //updated
	else if($gcond=='2,3,8'){ $gcond='2,3,8'; } //updated
	else if($gcond=='2,3,9'){ $gcond='2,3,9'; }  //updated
	...
	else if($gcond=='1,2,5,6'){ $gcond='2,5,6'; }
	else if($gcond=='1,2,6,5'){ $gcond='2,4,5,6'; }
	else if($gcond=='1,2,6,5,4'){ $gcond='2,4,6'; }
	else if($gcond=='1,2,4,5'){ $gcond='2,4'; }

	else if($gcond=='1,2,5,8'){ $gcond='2,4,6'; }

	else if($gcond=='1,3,6,5'){ $gcond='3,4,5,6'; }
	else if($gcond=='3,6,5'){ $gcond='3,4,5,6'; }
	else if($gcond=='3,6,5,4'){ $gcond='3,4,6'; }
	else if($gcond=='3,8,5'){ $gcond='3,8'; }
	else if($gcond=='3,5,8'){ $gcond='3,8'; }
	...
	else if($gcond=='1,5,4,8'){ $gcond='3,5,8'; }
	//else if($gcond=='1,5,7,8'){ $gcond='1'; }
	else if($gcond=='1,2,10'){ $gcond='2'; }
	else if($gcond=='5,9,6'){ $gcond='5,4,6,9'; }
	else if($gcond=='5,6,9'){ $gcond='5,4,6,9'; }

	else if($gcond=='1,5,9,6'){ $gcond='5,4,6,9'; }
	else if($gcond=='1,5,6,9'){ $gcond='5,4,6,9'; }
	else if($gcond=='3,5,9,6'){ $gcond='3,5,4,6,9'; }
	else if($gcond=='3,5,6,9'){ $gcond='3,5,4,6,9'; }
	else if($gcond=='1,3,5,9,6'){ $gcond='3,5,4,6,9'; }
	else if($gcond=='1,3,5,6,9'){ $gcond='3,5,4,6,9'; }
	else if($gcond=='1,2,5,9,6'){ $gcond='2,5,4,6,9'; }
	else if($gcond=='1,2,5,6,9'){ $gcond='2,5,4,6,9'; }	
	else if($gcond=='1,2,5,9,10,6'){ $gcond='2,5,4,6,9,10'; }
	else if($gcond=='1,2,5,6,9,10'){ $gcond='2,5,4,6,9,10'; }
...
//$conn->debug=true;
if(!empty($gcond)){ 
	//if (str_contains($gcond, '3')) {
		//sql cross check dgn SVM pemohon
	//}else{
		$sqlJ = "SELECT B.KOD as 'KOD', B.DISKRIPSI as 'DISKRIPSI'  
				FROM $schema2.`calon_jawatan_dipohon` A, $schema1.`ref_skim` B, $schema1.`padanan_peringkatakademik_skim` C  
				WHERE A.`kod_jawatan`=B.`KOD` 
				AND A.`kod_jawatan`=C.`kod_skim` 
				AND C.status=0";
			...
			<input type="hidden" name="id_pemohon" id="id_pemohon" value="<?php print $_SESSION['SESS_UID'];?>" readonly="readonly"/>
				...
				<div class="col-md-12">
					...
					if(!empty($gcond)){ 
                        if (str_contains($gcond, '3')) {
                            $sqlJ = "SELECT B.KOD as 'KOD', B.DISKRIPSI as 'DISKRIPSI' 
                            FROM $schema1.`padanan_peringkatakademik_skim` A
							join spp_ref.`ref_skim` B, spp_calon.calon_svm C  
					        WHERE A.`kod_skim`=B.`KOD` AND A.status=0 AND C.nama_sijil = A.kod_svm";
					        $sqlJ .= " AND A.`kod_peringkat_akademik`=3";
					        $sqlJ .= " AND C.id_pemohon = ".tosql($_SESSION['SESS_UID']);
					        if(!empty($dataj)){ $sqlJ .= " AND B.`KOD` NOT IN ($dataj)";  }
					        $sqlJ .= " UNION DISTINCT ";
							$sqlJ .= " SELECT B.KOD as 'KOD', B.DISKRIPSI as 'DISKRIPSI' 
                            FROM $schema1.`padanan_peringkatakademik_skim` A, $schema1.`ref_skim` B
					        WHERE A.`kod_skim`=B.`KOD` AND A.status=0";
					        $sqlJ .= " AND A.`kod_peringkat_akademik` IN ($gcond) AND A.kod_peringkat_akademik not in (1,3) ";
					        if(!empty($dataj)){ $sqlJ .= " AND B.`KOD` NOT IN ($dataj)";  }
					        $sqlJ .= " GROUP BY KOD ";
                            //print 'sqlJ='.$sqlJ;
							//exit;
					        ...
					        $sqlJ .= " AND A.`kod_peringkat_akademik` NOT IN (1,3)";
					        if(!empty($dataj)){ $sqlJ .= " AND B.`KOD` NOT IN ($dataj)";  }
					        $sqlJ .= " GROUP BY KOD ";
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="row">
				                        ...
				                        <?php
				                            ...
				                            <option title="<?php print $rsJawatan->fields['DISKRIPSI'];?>"
        										value="<?php print $rsJawatan->fields['KOD'];?>">
