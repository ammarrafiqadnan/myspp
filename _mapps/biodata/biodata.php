...
function do_save(val1, val2){
    ...
    if(ketinggian.trim() > 190 || ketinggian.trim() < 100){
        msg = msg+"\n- Maklumat ketinggian tidak tepat.";
        $('#ketinggian').css("border-color","#f00");
	}
    if(berat.trim() ==''){
    ...
    if(berat.trim() > 180 || berat.trim() < 30){
        msg = msg+"\n- Maklumat berat tidak tepat.";
        $('#berat').css("border-color","#f00");
	}
...
var validateH = function(e) {
	...
	if(t<191 && t>99){ 
		...
		alert("Sila pastikan ketinggian anda dalam sentimeter");
		document.myspp.ketinggian.value='';
	}
}
var validateB = function(e) {
 	var t = e.value;
  	if(t<=180 && t>=30){ 
		...
		alert("Sila pastikan berat anda dalam KG");
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-2">
								...
								<input type="" name="ketinggian" id="ketinggian" value="<?=$data['ketinggian']?>" class="form-control" onmouseleave="validateH(this)">
							</div>

							<div class="col-sm-2"></div>
							<label for="nama" class="col-sm-2 control-label"><b>Berat (KG) <font color="#FF0000">*</font><div style="float:right">:</div></b></label>
							<div class="col-sm-2">
								<input type="" name="berat" id="berat" value="<?=$data['berat'];?>" class="form-control" onmouseleave="validateB(this)">
							</div>
							<div class="col-sm-12" align="right">
                            	<i>* Bagi pemohon yang hamil, sila masukkan Berat (KG) sebelum mengandung.</i>
                        	</div>
						</div>
					</div>
					<div class="form-group">
                ...
                <div class="modal-content">
                    ...
                    <section class="panel">
                        <header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
                            ...
                            <h6 class="panel-title"><font color="#000000" size="3"><b>PEMAKLUMAN</b></font></h6>
                        </header>
                        <div class="panel-body">
                            <div class="box-body" align="center">
                            	<p style="text-align: centre;"><b>Keutamaan Tapisan Calon Dalam Urusan Pengambilan Dan Pelantikan</b></p><br>
								<p style="text-align: left;">Suruhanjaya Perkhidmatan Pendidikan (SPP) merupakan Pihak Berkuasa Melantik (PBM) dalam perkhidmatan pendidikan yang menjalankan fungsinya mengikut Perkara 144(1) Perlembagaan Persekutuan.<br>
								
								<p style="text-align: left;">Dalam urusan pengambilan dan pelantikan, SPP turut mendokong hasrat kerajaan dalam Agenda Nasional Malaysia Sihat (ANMS) yang menyasarkan masyarakat Malaysia mengamalkan budaya hidup sihat bagi meningkatkan kesejahteraan kesihatan.<br>

								<p style="text-align: left;">Sehubungan dengan itu, SPP menggunakan Indeks Jisim Badan atau Body Mass Index (BMI) yang dikeluarkan oleh World Health Organization (WHO) sebagai panduan dalam menyenarai pendek calon-calon bagi tujuan pengambilan dan pelantikan. Oleh yang demikian, 
									<b>hanya calon-calon yang memenuhi semua syarat yang ditetapkan di samping mempunyai BMI 35* dan ke bawah sahaja yang akan dipertimbangkan untuk ditemu duga dan ditawarkan pelantikan.</b><br>

								<p style="text-align: left;"><i><b>*kecuali calon yang sedang hamil tetapi mempunyai BMI 35 dan ke bawah sebelum hamil</i></b></p>
...
<script type="text/javascript">
  $(document).ready(function () {
    let beratPopupShown = false; // Flag to track if modal has been shown

    $('#berat').on('focus', function () {
      if (!beratPopupShown) {
        $('#myModalInfo').modal('show');
        beratPopupShown = true; // Prevent further popups
        $('#berat').blur(); // Remove focus to prevent editing while modal is open
      }
    });

    // When modal is closed (after clicking 'Tutup')
    $('#myModalInfo').on('hidden.bs.modal', function () {
      $('#berat').focus(); // Focus again to allow editing after modal closes
    });
  });
</script>