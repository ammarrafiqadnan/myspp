...
if($frm=='SRP'){
	...
	if($pro=='UPDATE'){
		...
		for($i=0;$i<=11;$i++){
			...
			if(!empty($mp[$i]) && !empty($gred[$i])){
				$rs = $conn->query("SELECT * FROM $schema2.`calon_srp` WHERE `jenis_sijil`='1' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($srp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]));
				if($rs->EOF){
					...
					$sql .= " VALUES('{$srp_id}', ".tosql($id_pemohon).",".tosql($srp_tahun_pilih).",".tosql($mp[$i]).",".tosql($jsijil).",".tosql($gred[$i]).",".tosql($dt).")";
				} else {
					$sql = "UPDATE $schema2.`calon_srp` SET `matapelajaran`=".tosql($mp[$i]).", `gred`=".tosql($gred[$i]).", d_kemaskini=".tosql($dt)." WHERE `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($srp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]);
				}
	...
	if($pro=='UPDATE'){
		...
		if(!empty($actions) && $actions=='1'){ 
			...
			$sql = "UPDATE $schema2.calon  SET spm_tahun_1=".tosql($spm_tahun_1).", spm_jenis_sijil_1=".tosql($spm_jenis_sijil_1).", 
			spm_lisan_1=".tosql($spm_lisan_1)." WHERE `id_pemohon`=".tosql($id_pemohon);
			...
			$sql = "UPDATE $schema2.calon  SET spm_tahun_2=".tosql($spm_tahun_1).", spm_jenis_sijil_2=".tosql($spm_jenis_sijil_1).", 
			spm_lisan_2=".tosql($spm_lisan_1)."  WHERE `id_pemohon`=".tosql($id_pemohon);
		...
		if($actions==1){ 
			...
			$sql = "UPDATE $schema2.calon  SET spm_tahun_1='{$spm_tahun_pilih}', spm_jenis_sijil_1='{$spm_sijil_pilih}', spm_pangkat_1='{$spm_pangkat_1}' WHERE  `id_pemohon`=".tosql($id_pemohon);
		} else if($actions==2){ 
			$spm_jenis_sijil_2=isset($_REQUEST["spm_jenis_sijil_2"])?$_REQUEST["spm_jenis_sijil_2"]:"";
			$spm_pangkat_2=isset($_REQUEST["spm_pangkat_2"])?$_REQUEST["spm_pangkat_2"]:"";
			$sql = "UPDATE $schema2.calon  SET spm_tahun_2='{$spm_tahun_pilih}', spm_jenis_sijil_2='{$spm_sijil_pilih}', spm_pangkat_2='{$spm_pangkat_1}' WHERE  `id_pemohon`=".tosql($id_pemohon);
		}
		// print $sql; exit;
		...
		if(!empty($actions) && $actions==1){
			...
			for($i=0;$i<=10;$i++){
				...
				if(!empty($mp[$i]) && !empty($gred[$i])){
					...
					$rs = $conn->query("SELECT * FROM $schema2.`calon_stp_stam` WHERE `jenis_xm`='B' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($stp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]));
					if($rs->EOF){
						...
						$sql .= " VALUES('{$stpm_id}', ".tosql($id_pemohon).",".tosql($stp_tahun_pilih).",".tosql($mp[$i]).",".tosql($jsijil).",".tosql($gred[$i]).",".tosql($dt).", 'B')";
					} else {
						$sql = "UPDATE $schema2.`calon_stp_stam` SET `gred`=".tosql($gred[$i]).", d_kemaskini=".tosql($dt)." WHERE `jenis_xm`='B' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($stp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]);
					}
					$conn->execute($sql);
				}
				$err='OK';
		...
		if(!empty($actions) && $actions==2){
			...
			for($i=0;$i<=10;$i++){
				...
				if(!empty($mp[$i]) && !empty($gred[$i])){
					...
					$rs = $conn->query("SELECT * FROM $schema2.`calon_stp_stam` WHERE `jenis_xm`='BT' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($stp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]));
					if($rs->EOF){
						$stpm_id = date("YmdHisN").uniqid();
						$sql = "INSERT INTO $schema2.`calon_stp_stam`(`stp_id`, `id_pemohon`, `tahun`, `matapelajaran`, `jenis_sijil`, `gred`, `d_cipta`, `jenis_xm`)";
						$sql .= " VALUES('{$stpm_id}', ".tosql($id_pemohon).",".tosql($stp_tahun_pilih).",".tosql($mp[$i]).",".tosql($jsijil).",".tosql($gred[$i]).",".tosql($dt).", 'BT')";
					} else {
						$sql = "UPDATE $schema2.`calon_stp_stam` SET `gred`=".tosql($gred[$i]).", d_kemaskini=".tosql($dt)." WHERE `jenis_xm`='BT' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($stp_tahun_pilih)." AND `matapelajaran`=".tosql($mp_old[$i]);
					}
					$conn->execute($sql);
				}
				$err='OK';
	...
	if($pro=='UPDATE'){
		...
		// print_r($mp);
		// print "tahun:".$stam_tahun_1.":".$stam_tahun_2;
		...
		if(!empty($stam_tahun_2)){ 
			// print "#######";
			// $conn->debug=true;
			$mp_1=isset($_REQUEST["mp_1"])?$_REQUEST["mp_1"]:"";
			$gred_1=isset($_REQUEST["gred_1"])?$_REQUEST["gred_1"]:"";
			$type='STAM2';
			$subj='';
			for($i=0;$i<=6;$i++){
				if($i==0){ 
					if(!empty($mp_1[$i])){ $subj .= "'$mp_1[$i]'"; }
				} else {
					if(!empty($mp_1[$i])){ $subj .= ",'$mp_1[$i]'"; }
				}
				// if(!empty($mp_2[$i])){ $subj .= ",'$mp_2[$i]'"; }
				$dt = date("Y-m-d H:i:s");
				if(!empty($mp_1[$i]) && !empty($gred_1[$i])){
					...
					if($rs->EOF){
						$sql = "INSERT INTO $schema2.`calon_stp_stam`(`id_pemohon`, `tahun`, `matapelajaran`, `jenis_sijil`, `gred`, `d_cipta`, `jenis_xm`)";
						$sql .= " VALUES(".tosql($id_pemohon).",".tosql($stam_tahun_2).",".tosql($mp_1[$i]).",".tosql($jsijil).",".tosql($gred_1[$i]).",".tosql($dt).", 'AT')";
					} else {
						$sql = "UPDATE $schema2.`calon_stp_stam` SET `gred`=".tosql($gred_1[$i]).", d_kemaskini=".tosql($dt)." WHERE `jenis_xm`='AT' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($stam_tahun_2)." AND `matapelajaran`=".tosql($mp_1[$i]);
					}
					$conn->execute($sql);
				}
				$err='OK';
	...
	if($pro=='ADD'){
		...
		if($kep==1){
			$jsijil1 = 'UNIV1A'; $jsijil2 = 'UNIV1B'; 
		} else if($kep==2){
			$jsijil1 = 'UNIV2A'; $jsijil2 = 'UNIV2B'; 
		} else if($kep==3){
			$jsijil1 = 'UNIV3A'; $jsijil2 = 'UNIV3B'; 
		}
		...
		if(!empty($bil_keputusan1)){ 
			$sql = "UPDATE $schema2.`calon_ipt` SET `tahun`=".tosql($tahun1).", `peringkat`=".tosql($peringkat1).", `cgpa`=".tosql($cgpa1).", 
				...
				`pengkhususan`=".tosql($pengkhususan1).", `biasiswa`=".tosql($biasiswa1).", `tkh_senate`=".tosql($tkh_senate).",  
			...
			VALUES(".tosql($id_pemohon).", '1', ".tosql($tahun1).", ".tosql($peringkat1).", ".tosql($cgpa1).", ".tosql($inst_keluar_sijil1).", 
				...
				".tosql($tkh_senate).", ".tosql(date("Y-m-d H:i:s")).")";
		}
	...
	if(!empty($_FILES['file1']['name'])){ 
        ...
        $final_image = strtolower($id_pemohon."_".$typeA.".".$ext);
	...
	if(!empty($_FILES['file2']['name'])){ 
        ...
        $final_image = strtolower($id_pemohon."_".$typeB.".".$ext);
	...
	if(!empty($_FILES['file3']['name'])){ 
        ...
        $final_image = strtolower($id_pemohon."_".$typeC.".".$ext);
	...
	if($pro=='SAVE'){ 
		...
		if(!empty($id_pemohon)){
			...
			if(!empty($tahun1) && !empty($mp1) && !empty($gred1)){
				$sql='';
					$sql = "INSERT INTO $schema2.`calon_spm`(`id_pemohon`, `tahun`, `matapelajaran`, `gred`, `jenis_xm`, `jenis_sijil`, `ujian_lisan`, `d_cipta`, `d_kemaskini`)";
					$sql .= " VALUES('{$id_pemohon}', ".tosql($tahun1).", ".tosql($mp1).", ".tosql($gred1).", 'T', ".tosql($bm1).", ".tosql($lisan1).", '{$date}', '{$date}')";
				if(!empty($sql)){ 
					$rss = $conn->execute($sql); 
				}
			}
			...
			if(!empty($tahun2) && !empty($mp2) && !empty($gred2)){
				$sql='';
					$sql = "INSERT INTO $schema2.`calon_spm`(`id_pemohon`, `tahun`, `matapelajaran`, `gred`, `jenis_xm`, `jenis_sijil`, `ujian_lisan`, `d_cipta`, `d_kemaskini`)";
					$sql .= " VALUES('{$id_pemohon}', ".tosql($tahun2).", ".tosql($mp2).", ".tosql($gred2).", 'T', ".tosql($bm2).", ".tosql($lisan2).", '{$date}', '{$date}')";
				if(!empty($sql)){ 
					$rss = $conn->execute($sql); 
				}
			}
			...
			if(!empty($tahun3) && !empty($mp3) && !empty($gred3)){
				$sql='';
					$sql = "INSERT INTO $schema2.`calon_spm`(`id_pemohon`, `tahun`, `matapelajaran`, `gred`, `jenis_xm`, `jenis_sijil`, `ujian_lisan`, `d_cipta`, `d_kemaskini`)";
					$sql .= " VALUES('{$id_pemohon}', ".tosql($tahun3).", ".tosql($mp3).", ".tosql($gred3).", 'T', ".tosql($bm3).", ".tosql($lisan3).", '{$date}', '{$date}')";
				if(!empty($sql)){ 
					$rss = $conn->execute($sql); 
				}
			}
			...
			if(!empty($tahun4) && !empty($mp4) && !empty($gred4)){
				$sql='';
					$sql = "INSERT INTO $schema2.`calon_spm`(`id_pemohon`, `tahun`, `matapelajaran`, `gred`, `jenis_xm`, `jenis_sijil`, `ujian_lisan`, `d_cipta`, `d_kemaskini`)";
					$sql .= " VALUES('{$id_pemohon}', ".tosql($tahun4).", ".tosql($mp4).", ".tosql($gred4).", 'T', ".tosql($bm4).", ".tosql($lisan4).", '{$date}', '{$date}')";
				if(!empty($sql)){ 
					$rss = $conn->execute($sql); 
				}
			}
		...
		// 	$sqlu = "UPDATE $schema2.`calon_spm` SET `professional_1`=NULL, `professional_d_1`=NULL, `professional_no_ahli_1`=NULL";
		// 	$sqlu .= " WHERE `id_pemohon`=".tosql($id_pemohon);
		// 	$rss = $conn->execute($sqlu);
		//  AND `jenis_sijil`='{$j}'
			$rs = $conn->query("SELECT * FROM $schema2.`calon_spm` WHERE `jenis_xm`='T' AND `id_pemohon`='{$id_pemohon}' AND `tahun`='{$t}' AND `matapelajaran`='{$mp}'");
			if(!$rs->EOF){
				$sqld = "DELETE FROM $schema2.`calon_spm` WHERE `jenis_xm`='T' AND `id_pemohon`='{$id_pemohon}' AND `tahun`='{$t}' AND `matapelajaran`='{$mp}'";
	...
	if($pro=='SAVE'){
		...
		// print_r($mp); exit;
		$rs = $conn->query("SELECT * FROM $schema2.`calon_svm` WHERE `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($spm_tahun_pilih));
		if($rs->EOF){
			...
			$sql .= " VALUES(".tosql($id_pemohon).",".tosql($spm_tahun_pilih).",".tosql($spm_sijil_pilih).",".tosql($svm_sijil_1).",".tosql($gred_bm_1).",".tosql($svm_pngk).", ".tosql($svm_pngkv).", ".tosql($date).", ".tosql($date).")";
		} else {
			$sql = "UPDATE $schema2.`calon_svm` SET `tahun`=".tosql($spm_tahun_pilih).", `jenis_sijil`=".tosql($spm_sijil_pilih).", 
				...
				WHERE `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($spm_tahun_pilih);
		}
		...
		if(!empty($_FILES['file_pmr']['name'])){ 
			...
			if(in_array($ext, $valid_extensions)){ 
				...
				$rsu = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='SVM1' AND `id_pemohon`=".tosql($id_pemohon));
				if($rsu->EOF){ 
					$sqlu = "INSERT INTO $schema2.`calon_sijil`(`id_pemohon`, `jenis_sijil`, `sijil_nama`, `sijil_size`, `sijil_type`, `dt_create`) 
					VALUES('{$id_pemohon}', 'SVM1', '{$final_image}', ".tosql($sijil_size).", '{$sijil_type}', '{$dt}')";
				} else {
					$sqlu = "UPDATE $schema2.`calon_sijil` SET `sijil_nama`='{$final_image}', `sijil_size`=".tosql($sijil_size).", 
					`sijil_type`=".tosql($sijil_type).", `dt_update`='{$date}' WHERE `jenis_sijil`='SVM1' AND `id_pemohon`=".tosql($id_pemohon);				
				}			    
				$rsn = $conn->execute($sqlu);		
				if($rsn){ 	
				    $err='OK';
		...
		if(!empty($id_pemohon)){
			...
			$sqlu = "DELETE FROM $schema2.`calon_svm` WHERE `id_pemohon`=".tosql($id_pemohon);
			$rss = $conn->execute($sqlu);

			$rs = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='SVM' AND `id_pemohon`='{$id_pemohon}'");
			if(!$rs->EOF){
				$sqld = "DELETE FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='SVM' AND `id_pemohon`='{$id_pemohon}'";
				$path = '../../uploads_doc/'.$id_pemohon.'/'.$rs->fields['sijil_nama']; // upload directory
				$conn->execute($sqld);
				unlink($path);
			}
