				...
				<div class="col-md-12">
					...
					<?php } ?>
									...
									if(empty($data_pmr['srp_tahun'])){ 
										...
										$conn->execute("UPDATE $schema2.`calon` SET `srp_jenis_sijil`=NULL WHERE `id_pemohon`=".tosql($uid));
									} else {
					...
					<?php } ?>
									...
									if(empty($data_spm['spm_tahun_1'])){ 
										...
										$conn->execute("UPDATE $schema2.`calon` SET `spm_jenis_sijil_1`=NULL WHERE `id_pemohon`=".tosql($uid));
									} else {
										// KENA CHECK BY TAHUN
					...
					<?php } ?>
									...
									$sqlp = "SELECT * FROM $schema2.`calon_pusat_temuduga` WHERE id_pemohon=".tosql($_SESSION['SESS_UID']);
									$rsp = $conn->query($sqlp);
									if(!$rsp->EOF){ 	
									   if(!empty($rsp->fields['pusat_temuduga'])){
										$JawatanD='Lengkap'; //print "..";
									   } else {
										$bl=''; print "...";
										$JawatanD='Sila lengkapkan maklumat berkaitan Pusat Temu Duga di bahagian "Jawatan Dimohon"';
									   }
									} else {
										$bl=''; print "...";
										$JawatanD='Sila lengkapkan maklumat berkaitan Pusat Temu Duga di bahagian "Jawatan Dimohon"';
