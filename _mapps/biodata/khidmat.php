...
function do_save_khidmat(val1, val2){
	...
	//var GivenDate = Date(d_lantikan_khidmat_sekarang);
	var todayDate = new Date().toISOString().slice(0, 10);
	//console.log(todayDate+":"+getDate+":"+GivenDate+":"+CurrentDate);
	//document.myspp.dates.value = todayDate+":"+getDate+":"+GivenDate+":"+CurrentDate;
	if(d_lantikan_khidmat_sekarang.trim()!=''){
		//getDate = new Date(d_lantikan_khidmat_sekarang).toISOString().slice(0, 10);
		//getDate=formatDate(d_lantikan_khidmat_sekarang.trim());
		const myArray = d_lantikan_khidmat_sekarang.split("/");
		getDate=myArray[2]+"-"+myArray[1]+"-"+myArray[0];
	}
	
	todayDate=formatDate(todayDate);

	//alert(CurrentDate+":"+GivenDate);

	if(jenis_perkhidmatan.trim() ==''){
        msg = msg+'- Sila pilih jenis perkhidmatan.';
        $("#jenis_perkhidmatan").css("border-color","#f00");
		$(".jenis_perkhidmatan").each(function() {
			$(this).siblings(".select2").css('border', '1px solid red');
		});
    ...
    if(taraf_jawatan.trim() ==''){
        ...
        $("#taraf_jawatan").css("border-color","#f00");
			$(".taraf_jawatan").each(function() {
				$(this).siblings(".select2").css('border', '1px solid red');
			});

    }
    if(taraf_jawatan.trim()=='3' || taraf_jawatan.trim()=='5' || taraf_jawatan.trim()=='6'){
		if(d_lantikan_kontrak.trim()==''){ 
    	   	msg = msg+"\n- Sila masukkan tarikh tamat lantikan kontrak.";
        	//$("#d_lantikan_kontrak").css("border-color","#f00");
			$(".taraf_jawatan").each(function() {
				$(this).siblings(".select2").css('border', '1px solid red');
			});
		}
    ...
    if(skim_sekarang.trim() ==''){
        msg = msg+"\n- Sila pilih skim perkhidmatan sekarang.";
        //$('#skim_sekarang').css("border-color","#f00");
		$(".skim_sekarang").each(function() {
			$(this).siblings(".select2").css('border', '1px solid red');
		});
	}
    if(gred_jawatan_sekarang.trim() ==''){
		...
		$(".gred_jawatan_sekarang").each(function() {
			$(this).siblings(".select2").css('border', '1px solid red');
		});

	}
    if(d_lantikan_khidmat_sekarang.trim() ==''){
        msg = msg+"\n- Sila pilih/masukkan tarikh lantikan ke skim perkhidmatan sekarang.";
        $('#d_lantikan_khidmat_sekarang').css("border-color","#f00");
	}
    	
	if(getDate.trim()!=''){
	if(getDate.trim() > todayDate.trim()){
        msg = msg+"\n- Maklumat bagi tarikh lantikan ke skim perkhidmatan sekarang lebih besar daripada tarikh semasa."+getDate+":"+todayDate;
        $('#d_lantikan_khidmat_sekarang').css("border-color","#f00");
	}
	}
	...
	if(taraf_jawatan == 1){
	 	if(d_sah_khidmat_sekarang.trim() ==''){
        	msg = msg+"\n- Sila pilih/masukkan tarikh pengesahan ke skim perkhidmatan sekarang.";
    ...
    if(negeri_tpt_bertugas.trim() ==''){
		...
		$(".negeri_tpt_bertugas").each(function() {
			$(this).siblings(".select2").css('border', '1px solid red');
		});
	}
				...
				<div class="col-md-12">
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-5">
								...
								<select name="jenis_perkhidmatan" id="jenis_perkhidmatan" class="form-control select2 jenis_perkhidmatan" style="width: 100%;" aria-hidden="true">
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-5">
								<select name="taraf_jawatan" id="taraf_jawatan" class="form-control select2 taraf_jawatan" style="width: 100%;" aria-hidden="true">
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-3">
								...
								data-inputmask-alias="datetime" data-inputmask-inputformat="dd/mm/yyyy" data-mask 
								class="form-control disableFuturedate2 icon" style="background-color: #fff; cursor: pointer;">
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-5">
								<select name="skim_sekarang" id="skim_sekarang" class="form-control select2 select2-accessible skim_sekarang" onchange="get_gred(this.value)" 
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-5">
								<select name="gred_jawatan_sekarang" id="gred_jawatan_sekarang" class="form-control select2 select2-accessible gred_jawatan_sekarang" style="width: 100%;" aria-hidden="true">
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-sm-5">
								<select name="tpt_bertugas" id="tpt_bertugas" class="form-control select2 select2-selection3 tpt_bertugas">
									<option value="">Sila pilih kemeterian/agensi tempat bertugas</option>
					...
					<div class="form-group">
						<div class="row">
							<label for="nama" class="col-sm-7 control-label"><b>Negeri Tempat Bertugas  <font color="#FF0000">*</font>
								<div style="float:right"><!--<img src="../images/info.gif" title="Sila klik pada medan carian dan calon boleh menaip perkataan yang dikehendaki. Carian dibuat berdasarkan perkataan yang ditaip.">--> :</div></b></label>
							<div class="col-sm-5">
								<select name="negeri_tpt_bertugas" id="negeri_tpt_bertugas" class="form-control select2 tpt_bertugas">
...
<input type="hidden" name="dates" width="500px" value="<?php print $proses;?>" />	     
