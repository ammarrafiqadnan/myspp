...
if($frm=='MOHON'){
	...
	if($pro=='SAVE'){
		...
		VALUES('{$id_pemohon}', 'NULL', '{$peringkat}', '{$dt_mohon}', '{$pusat_temuduga}', '01', '{$date}')";
		$conn->execute($sql2);
	...
	if($rss){
		...
		$sql = "UPDATE $schema2.`calon` SET `pengakuan`='Y', `tarikh_akuan`=".tosql(date("Y-m-d H:i:s"))." WHERE `id_pemohon`=".tosql($id_pemohon);
		$rss = $conn->execute($sql);
		...
		if($err=='OK'){
			...
			$sql = "UPDATE $schema2.`calon_tarikh` SET `pengakuan`='Y', `tarikh_akuan`=".tosql(date("Y-m-d H:i:s")).", `tkh_upd_perakuan`=".tosql(date("Y-m-d H:i:s")).", 
				`status_pemohon`='Y' WHERE `id_pemohon`=".tosql($id_pemohon);
			$rss = $conn->execute($sql);

			//$rs = $conn->query("INSERT INTO `calon_tarikh_sejarah` SELECT * FROM `calon_tarikh` WHERE `id_pemohon`=".tosql($id_pemohon));
			$rs = $conn->query("SELECT * FROM `calon_tarikh` WHERE `id_pemohon`=".tosql($id_pemohon));
			$sql = "INSERT INTO `calon_tarikh_sejarah`(`id_pemohon`, `tkh_upd_biodata`, `tkh_upd_akademik`, `tkh_upd_koko`, 
			`tkh_upd_awam`, `tkh_upd_tambahan`, `tkh_upd_jawatan`, `tkh_upd_perakuan`, 
			...
			".tosql($rs->fields['tkh_upd_awam']).", ".tosql($rs->fields['tkh_upd_tambahan']).", ".tosql($rs->fields['tkh_upd_jawatan']).", ".tosql($rs->fields['tkh_upd_perakuan']).", 
