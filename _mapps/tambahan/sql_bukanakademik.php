...
if($frm=='POLIS'){
	...
	if($pro=='SAVE'){
		...
		if($rss){
			...
			$sqli = "UPDATE $schema2.`calon` SET `kategori_tentera_polis`=".tosql($kategori).", `pencen`=".tosql($ganjaran)." WHERE `id_pemohon`=".tosql($id_pemohon);
			$conn->execute($sqli);
	...
	if($pro=='SAVE'){ 
		...
		if(!$rss->EOF){
			$sql = "UPDATE $schema2.`calon_ko_rekacipta` SET `rekacipta`=".tosql($rekacipta).", `sumbangan`=".tosql($sumbangan).", 
			`peringkat`=".tosql($peringkat).", `d_kemaskini`='{$date}' 
			...
			if(!empty($rekacipta) && !empty($sumbangan) && !empty($peringkat)){ 
				...
				VALUES('{$id_pemohon}', ".tosql($rekacipta).", ".tosql($sumbangan).", ".tosql($peringkat).", '{$date}', '{$date}')";
				$rss = $conn->execute($sql);
			}
		}
	...
	if($pro=='SAVE'){
		...
		// SEMAKAN UNTUK PLKN
		$plkn=isset($_REQUEST["plkn"])?$_REQUEST["plkn"]:"";
		$rs = $conn->query("SELECT * FROM $schema2.`calon_ko_plkn` WHERE `id_pemohon`=".tosql($id_pemohon));
		if(!$rs->EOF){
			$conn->execute("UPDATE $schema2.`calon_ko_plkn` SET `plkn`='{$plkn}', `d_ubahsuai`='{$date}' WHERE `id_pemohon`=".tosql($id_pemohon));
		} else {
			$conn->execute("INSERT INTO $schema2.`calon_ko_plkn`(`id_pemohon`,`plkn`,`id_cipta`,`d_cipta`,`d_ubahsuai`) 
				VALUES (".tosql($id_pemohon).",'{$plkn}','MYSPP','{$date}', '{$date}')");
		}
...
set_tarikh($id_pemohon, "tkh_upd_koko");
