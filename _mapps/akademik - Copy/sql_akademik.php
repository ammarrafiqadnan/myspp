...
if($frm=='SRP'){
		...
		if(!empty($spm_tahun_2)){ 
			...
			for($i=0;$i<=11;$i++){
				...
				if(!empty($mp_2[$i]) && !empty($gred_2[$i])){
					$rs = $conn->query("SELECT * FROM $schema2.`calon_spm` WHERE `jenis_xm`='T' AND `id_pemohon`=".tosql($id_pemohon)." AND `tahun`=".tosql($spm_tahun_1)." AND `matapelajaran`=".tosql($mp_2[$i]));
					if($rs->EOF){
						$sql = "INSERT INTO $schema2.`calon_spm`(`id_pemohon`, `tahun`, `matapelajaran`, `jenis_sijil`, `gred`, `d_cipta`, `jenis_xm`)";
	...
	if($pro=='UPDATE'){
		...
		if($rss){
			$err='OK';
			$sqld = "DELETE FROM $schema2.`calon_spm` WHERE `jenis_xm`=='BT' AND `id_pemohon`=".tosql($id_pemohon);
			$conn->execute($sqld);
		} else {
			$err='ERR';
		}
		...
		if(!empty($_FILES['file_pmr']['name'])){ 
			...
			if(in_array($ext, $valid_extensions)){ 
				...
				$dt = date("Y-m-d H:i:s");
				$upd_main='';
				$sqlu = "INSERT INTO `calon_sijil`(`id_pemohon`, `jenis_sijil`, `sijil_nama`, `sijil_size`, `sijil_type`, `dt_create`) 
				VALUES('{$id_pemohon}', '{$type}', '{$final_image}', '{$sijil_size}', '{$sijil_type}', '{$dt}')";
