...
function do_hapus(ID){
	swal({
		...
		$.ajax({
			url:'utiliti/kluster_form_sql.php?pro=DEL&id='+ID, //&datas='+datas,
			type:'POST',
			//dataType: 'json',
			/*beforeSend: function () {
...
?>
		...
		<div class="box" style="background-color:#F2F2F2">
			...
			<header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
                ...
                <h6 class="panel-title"><font color="#000000"><b>Maklumat Rujukan Kluster/Bidang</b></font></h6> 
            </header>
			</div>
            </div>            
            <br />
                ...
                <div class="form-group">
        			...
        			<div class="col-md-4" align="right">
                        <button type="button" class="btn btn-success" 
                            ...
                            <a href="utiliti/kluster_form.php" 
                            data-toggle="modal" data-target="#myModal"  
                            title="Tambah Maklumat Rujukan Kluster/Bidang" class="fa" data-backdrop="static">
			...
			<?php
			// $conn->debug=true;
            $sql = "SELECT * FROM _ref_kluster WHERE is_deleted=0 ";
            if(!empty($carian)){ $sql .= " AND kluster_nama LIKE '%{$carian}%'"; }
            $sql .= " ORDER BY kluster_nama";
			?>    
			...
			if(!$rs->EOF) {
				...
				while(!$rs->EOF){ 
					...
					$ids=$rs->fields['kluster_id'];
					$topik_nama = str_replace($carian,'<font color="#FF0000">'.$carian.'</font>', $rs->fields['kluster_nama']);
				?>
				  <tr>
					  <td align="center"><?=$bil;?></td>
					  <td align="left"><?php print $topik_nama;?></td>
					  <td align="center" class="actions">
						<?php if(empty($rs->fields['kluster_status'])){ ?>
					  ...
					  <td class="actions" align="center">
						  <a href="utiliti/kluster_form.php?ids=<?=$ids;?>" 
							data-toggle="modal" data-target="#myModal"  
							title="Kemaskini maklumat kluster/bidang" class="fa" data-backdrop="static" style="margin:0px;">
							...
							<button type="button" class="btn btn-sm btn-danger" onclick="do_hapus('<?=$ids;?>')">
								<span style="cursor:pointer;color:red" title="Hapus maklumat rujukan kluster/bidang">
