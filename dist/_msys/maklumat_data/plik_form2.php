    ...
    <?php if(!empty($id)){ ?>
      <!-- <button type="button" class="btn btn-info mt-sm mb-sm" title="Tambah Maklumat"> -->
        ...
        <?php if($_SESSION['SESS_ULEVEL']==2 || $_SESSION['SESS_ULEVEL']==3){ ?>
	        <a href="maklumat_data/plik_form_info.php?id=<?=$id;?>&ids=<?=$id_det;?>" data-toggle="modal" data-target="#myModal" 
						title="Kemaskini maklumat tambahan/pemantauan" class="fa" data-backdrop="">
						<label class="btn btn-success"><i class="fa fa-plus" style="color:white;font-family: Arial;"> Tambah Maklumat</i></label>
					</a>
				<?php } ?>
...
<?php 
			// $conn->debug=true;
			$sql = "SELECT * FROM `tbl_plik_info` WHERE is_deleted=0 AND `plik_id`='{$id}' ORDER BY info_tarikh DESC"; 
			$kbil=0; $tbil=0;
			$rs = $conn->query($sql);
            ?>    
            ...
            <?php 
				...
				?>
				  <tr>
					  ...
					  <td align="center">
					  	<?php if($_SESSION['SESS_ULEVEL']==2 || $_SESSION['SESS_ULEVEL']==3){ ?>
					  		<a href="maklumat_data/plik_form_info.php?id=<?=$id;?>&id_det=<?=$id_det;?>" data-toggle="modal" data-target="#myModal" 
									title="Kemaskini maklumat tambahan/pemantauan" class="fa" data-backdrop="static">
									<label class="btn btn-sm btn-success">
										<i class="fa fa-edit" style="color:white;"></i>
									</label></a>
								&nbsp;
								<button type="button" class="btn btn-sm btn-danger" onclick="do_delete2('maklumat_data/sql_maklumat_data.php?frm=PLIK_INFO&pro=DEL&id_det=<?=$id_det;?>')">
