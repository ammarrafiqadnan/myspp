...
<script>
    ...
    function do_check(kod_institusi){
        ...
        $.ajax({
            url: 'pengurusan/sql_pengurusan1.php?frm=PARAMETER&jenis=PADANAN_PERINGKATKELULUSAN_INSTITUSI&pro=SAVE&peringkat2='+peringkat2+'&chk='+chks+'&kod_institusi='+kod_institusi+'&kategori='+kategori,
            type:'POST',
            //dataType: 'json',
            data: $("form").serialize(),
            //data: datas,
...
if($kategori=='AWAM'){ 
    $tajuk= 'AWAM';
    $sql3 = "SELECT A.*, B.DISKRIPSI, B.KATEGORI, B.KOD FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B 
    WHERE A.`kod_institusi`=B.`KOD` AND A.`status`=0 AND B.`is_deleted`=0 AND B.`NEGARA`='130' AND B.`SAH_YT`='Y'";
} else if($kategori=='SWASTA'){ 
    $tajuk= 'SWASTA';
    $sql3 = "SELECT A.*, B.DISKRIPSI, B.KATEGORI, B.KOD FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B 
    WHERE A.`kod_institusi`=B.`KOD` AND A.`status`=0 AND B.`is_deleted`=0 AND B.`NEGARA`='130' AND B.`SAH_YT`='Y'";
} else if($kategori=='LUARNEGARA'){ 
    $tajuk= 'LUAR NEGARA';
    $sql3 = "SELECT A.*, B.DISKRIPSI, B.KATEGORI, B.KOD FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B 
    WHERE A.`kod_institusi`=B.`KOD` AND A.`status`=0 AND B.`is_deleted`=0 AND B.`SAH_YT`='Y' AND B.`NEGARA`!='130'";
}
if(!empty($peringkat2)){
...
if($kategori=='AWAM'){ 
    $sql = "SELECT * FROM $schema1.`ref_institusi` WHERE SAH_YT='Y' AND is_deleted=0 AND `JENIS_INSTITUSI`='1' AND `NEGARA`='130' ";
} else if($kategori=='SWASTA'){ 
    $sql = "SELECT * FROM $schema1.`ref_institusi` WHERE SAH_YT='Y' AND is_deleted=0 AND `JENIS_INSTITUSI`='2'  AND `NEGARA`='130' ";
} else if($kategori=='LUARNEGARA'){ 
    $sql = "SELECT * FROM $schema1.`ref_institusi` WHERE SAH_YT='Y' AND is_deleted=0 AND `NEGARA`!='130' ";
}
		...
		<div class="box" style="background-color:#F2F2F2">
            ...
            <div class="box-body">
                ...
                <header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
                    ...
                    <h6 class="panel-title"><font color="#000000"><b>Senarai Institusi <?=$tajuk;?></b></font></h6> 
                </header>
                </div>
            </div>    
                ...
                <div class="box-body" style="background-color:#F2F2F2; padding: 15px;">
                    <table id="example" class="table table-striped table-bordered" style="width:100%">
                        <thead  style="background-color:rgb(38, 167, 228)">
                            ...
                            <th width="10%"><font color="#000000"><div align="center">Kod Institusi</div></font></th>
                            <th width="65%"><font color="#000000"><div align="center">Nama Institusi</div></font></th>
                        ...
                        <?php 
                            ...
                            <tr>
                                ...
                                <td><?=$rsInstitusiUncheck->fields['KOD'];?></td>
