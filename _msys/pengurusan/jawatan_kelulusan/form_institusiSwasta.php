...
// $conn->debug=true;
$sql3 = "SELECT A.*, B.DISKRIPSI, B.KATEGORI, B.KOD FROM $schema1.`padanan_peringkatkelulusan_institusi` A, $schema1.`ref_institusi` B WHERE A.`kod_institusi`=B.`KOD` AND A.`status`=0 AND A.is_deleted=0 AND B.`SAH_YT`='Y' AND B.`KATEGORI` IS NULL";
...
$sql = "SELECT * FROM $schema1.`ref_institusi` WHERE SAH_YT='Y' AND KATEGORI IS NULL";
