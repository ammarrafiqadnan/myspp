...
if($frm=='PENGGUNA'){
	...
	if($jenis=='BANGSA'){
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				$strSQL = "UPDATE $schema1.`ref_pengkhususan` SET is_deleted=0 AND DISKRIPSI=".tosql($pengkhususan).", STATUS=".tosql($status_pengkhususan).", 
					NO_PEMEROLEHAN=".tosql($no_pemerolehan).", TARIKH_UBAHSUAI=".tosql($date).", ID_PENCIPTA=".tosql($oleh); 	
				$strSQL .= " WHERE kod=".tosql($kod);
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				$strSQL = "UPDATE $schema1.`ref_kecacatan_calon` SET DISKRIPSI=".tosql($oku).", status=".tosql($status_oku).", TARIKH_UBAHSUAI=".tosql($date).", ID_PENCIPTA=".tosql($oleh); 	
				$strSQL .= " WHERE KOD=".tosql($kod);
