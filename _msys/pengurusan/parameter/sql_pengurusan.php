...
if($frm=='PENGGUNA'){
	...
	if($pro == 'SAVE'){
		...
		if(!empty($id)){
			if($jenis == 'admin'){
				$strSQL = "UPDATE $schema2.spa8i_admin SET username=".tosql($noKP).",noKP=".tosql($noKP).",nama_penuh=".tosql($nama_penuh).",emel=".tosql($email).",bahagian=".tosql($bahagian2).",jawatan=".tosql($jawatan2).",no_tel=".tosql($no_tel).",peranan=".tosql($peranan2).",status=".tosql($status2);
			} else {
				$strSQL = "UPDATE $schema2.spa8i_admin SET username=".tosql($noKP).",noKP=".tosql($noKP).",nama_penuh=".tosql($nama_penuh).",emel=".tosql($email).",bahagian=".tosql($bahagian2).",jawatan=".tosql($jawatan2).",no_tel=".tosql($no_tel);
			} 	
			...
			$sql .= " VALUES(".tosql($noKP).",".tosqln($pwd).",".tosql($noKP).",".tosql($nama_penuh).",".tosql($email).",".tosql($bahagian2).",".tosql($jawatan2).",".tosql($no_tel).",".tosql($status2).",".tosql($peranan2).")";
			$rss = $conn->Execute($sql);
			audit_trail($sql, "TAMBAH BARU PENGGUNA");
		}
		...
		$rss = $conn->Execute($sql);
		audit_trail($sql, "HAPUS PENGGUNA");
	...
	if($jenis=='BANGSA'){
		...
		if($pro == 'SAVE'){
			...
			if(($rs->fields['total']) >= 1){
				$strSQL = "UPDATE $schema1.`ref_institusi` SET DISKRIPSI=".tosql($universiti).", STATUS=".tosql($status_universiti).", 
					...
					SAH_YT=".tosql($status_SAH_YT).", ID_PENCIPTA=".tosql($oleh); 	
				$strSQL .= " WHERE KOD=".tosql($kod);
		...
		if($pro == 'SAVE'){
			...
			//print 'sini'; exit();
			$conn->debug=true; 
			$sql3 = "SELECT COUNT(*) as total FROM $schema1.`ref_pengkhususan` A, $schema1.`padanan_institusi_pengkhususan` B 
				WHERE A.DISKRIPSI LIKE '$pengkhususan' AND A.is_deleted=0 AND A.kod=B.kod_pengkhususan AND B.kod_institusi=".tosql($institusi_kod);
			//$sql3 .= " AND 
			$sql3 = "SELECT COUNT(*) as total FROM $schema1.`ref_pengkhususan` A  
				WHERE A.DISKRIPSI LIKE '$pengkhususan' AND A.is_deleted=0 ";
			$rs = $conn->query($sql3);
			// print 'aaaaaaa'.$rs->fields['total'];
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				...
				TARIKH_UBAHSUAI=".tosql($date).", ID_PENCIPTA=".tosql($oleh); 	
				$strSQL .= " WHERE KOD=".tosql($kod);
				...
				$sql = "INSERT INTO $schema1.`ref_kecacatan_calon` (KOD, DISKRIPSI, `status`, SAH_YT, TARIKH_CIPTA) ";
				$sql .= " VALUES(".tosql($kod_save).",".tosql($oku).",".tosql($status_oku).", 'Y',".tosql($date).")";
				$rss = $conn->Execute($sql);
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				...
				$rss = $conn->execute($strSQL);
				audit_trail($strSQL, "KEMASKINI DOMAIN EMEL");
		...
		if($pro == 'SAVE'){
			...
			if(($rs->fields['total']) >= 1){
				$strSQL = "UPDATE $schema1.`ref_matapelajaran` SET DISKRIPSI=".tosql($matapelajaran).", STATUS=".tosql($status_matapelajaran).", NO_PEMEROLEHAN=".tosql($no_pemerolehan).", TKT=".tosql($tingkatan).", TARIKH_UBAHSUAI=".tosql($date).", ID_PENCIPTA=".tosql($oleh); 	
				$strSQL .= " WHERE kod=".tosql($kod);
				
				$rss = $conn->execute($strSQL);
				audit_trail($strSQL, "KEMASKINI MATAPELAJARAN");
				...
				$rss = $conn->Execute($sql);
				audit_trail($sql, "TAMBAH BARU MATAPELAJARAN");
			}
			...
			$sql = "UPDATE $schema1.`ref_matapelajaran` SET is_deleted=1 WHERE `kod`='{$kod}'"; 
			$rss = $conn->Execute($sql);
			audit_trail($sql, "HAPUS MATAPELAJARAN");
		...
		if($pro == 'SAVE'){
			...
			if(($rs->fields['total']) >= 1){
				$strSQL = "UPDATE $schema1.`ref_pusat_temuduga` SET diskripsi=".tosql($pusatTemuduga).", status=".tosql($status_pusatTemuduga).", neg_kod=".tosql($neg_kod).", tarikh_ubahsuai=".tosql($date).", id_pencipta=".tosql($oleh); 	
				$strSQL .= " WHERE kod=".tosql($kod);
				
				$rss = $conn->execute($strSQL);
				audit_trail($strSQL, "KEMASKINI PUSAT TEMU DUGA");
				...
				$rss = $conn->Execute($sql);
				audit_trail($sql, "TAMBAH BARU PUSAT TEMU DUGA");
			}
			...
			$sql = "UPDATE $schema1.`ref_pusat_temuduga` SET is_deleted=1 WHERE `kod`='{$kod}'"; 
			$rss = $conn->Execute($sql);
			audit_trail($sql, "HAPUS PUSAT TEMU DUGA");
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				$strSQL = "UPDATE $schema1.`ref_bidang` SET diskripsi=".tosql($bidang).", status=".tosql($status_bidang).", tarikh_ubahsuai=".tosql($date).", id_pencipta=".tosql($oleh); 	
				$strSQL .= " WHERE kod=".tosql($kod);
				
				$rss = $conn->execute($strSQL);
				audit_trail($strSQL, "KEMASKINI BIDANG");

			} else {
				$sql = "INSERT INTO $schema1.`ref_bidang` (kod, diskripsi, `status`, tarikh_cipta, id_pencipta) ";
				$sql .= " VALUES(".tosql($kod).",".tosql($bidang).",".tosql($status_bidang).",".tosql($date).",".tosql($oleh).")";
				$rss = $conn->Execute($sql);

				audit_trail($sql, "TAMBAH BARU BIDANG");
			}
			...
			$sql = "UPDATE $schema1.`ref_bidang` SET is_deleted=1 WHERE `kod`='{$kod}'"; 
			$rss = $conn->Execute($sql);

			audit_trail($sql, "HAPUS BIDANG");
		...
		if($pro == 'SAVE_ALL'){
			...
			if($check == 'yes'){
				...
				while(!$rsKluster->EOF) {
					...
					if($checkDiPadanan->fields['total'] > 0){
						$strSQL = "UPDATE $schema1.`padanan_kluster_bidang` SET `status`=0"; 	
						$strSQL .= " WHERE kod_kluster=".tosql($kluster)." AND kod_bidang=".tosql($rsKluster->fields['kod']);
						
						$rss = $conn->execute($strSQL);
						audit_trail($strSQL, "KEMASKINI PADANAN KLUSTER DAN BIDANG");
					} else {
						$sql = "INSERT INTO $schema1.`padanan_kluster_bidang` (kod_kluster, `kod_bidang`) ";
						$sql .= " VALUES(".tosql($kluster).",".tosql($rsKluster->fields['kod']).")";
						$rss = $conn->Execute($sql);
						audit_trail($sql, "TAMBAH BARU PADANAN KLUSTER DAN BIDANG");
					}
				...
				while(!$rsKluster->EOF){
					$strSQL = "UPDATE $schema1.`padanan_kluster_bidang` SET `status`=1"; 	
					$strSQL .= " WHERE kod_kluster=".tosql($kluster)." AND kod_bidang=".tosql($rsKluster->fields['KOD']);
			...
			if($chk == 'true'){
				...
				if($checkDiPadanan->fields['total']){
					...
					$sql = "INSERT INTO $schema1.`padanan_kluster_bidang` (kod_kluster, `kod_bidang`) ";
					$sql .= " VALUES(".tosql($kluster).",".tosql($kod_bidang).")";
					$rss = $conn->Execute($sql);
					audit_trail($strSQL, "TAMBAH BARU PADANAN KLUSTER DAN BIDANG");
				}
			} else {
			...
			$rss = $conn->execute($strSQL);
			audit_trail($strSQL, "HAPUS PADANAN KLUSTER DAN BIDANG");
		}
	...
	if($jenis == 'KAWALAN_MUATNAIK_DOKUMEN'){
		...
		if($pro == 'SAVE'){
			...
			if(!empty($kod)){
				...
				$rss = $conn->Execute($sql);
				audit_trail($sql, "TAMBAH BARU KAWALAN MUATNAIK DOKUMEN");
			...
			$rss = $conn->Execute($sql);
			audit_trail($sql, "HAPUS KAWALAN MUATNAIK DOKUMEN");
		...
		if($pro == 'SAVE'){
			...
			if(empty($id)){
				...
				$sql = "INSERT INTO $schema2.faq(tajuk, keterangan, `status`)";
				$sql .= " VALUES(".tosql($tajuk).", ".tosql($keterangan).", ".tosql($status).")";
				...
				$sql="UPDATE $schema2.faq SET tajuk=".tosql($tajuk).", keterangan=".tosql($keterangan).", `status`=".tosql($status)." WHERE kod='{$id}'";	
		...
		if($pro == 'SAVE'){
			...
			if(empty($id)){
				$sql = "INSERT INTO $schema2.kandungan_notifikasi(tajuk, kod_noti, `status`, keterangan)";
				$sql .= " VALUES(".tosql($tajuk).", ".tosql($kod_noti).", ".tosql($status).", ".tosql($keterangan).")";

				audit_trail($sql, "TAMBAH BARU KANDUNGAN NOTIFIKASI");

			} else {
				$sql="UPDATE $schema2.kandungan_notifikasi SET tajuk=".tosql($tajuk).", kod_noti=".tosql($kod_noti).", `status`=".tosql($status).", `keterangan`=".tosql($keterangan)." WHERE kod='{$id}'";	
		...
		if($pro == 'SAVE'){
			...
			if(empty($id)){
				...
				$sql="UPDATE $schema2.kandungan_surat SET tajuk=".tosql($tajuk).", keterangan=".tosql($keterangan).",  jenis=".tosql($jenis_surat).",  `status`=".tosql($status)." WHERE kod='{$id}'";
