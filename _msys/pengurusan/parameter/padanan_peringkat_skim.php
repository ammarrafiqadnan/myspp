...
<?php
    ...
    if ($peringkat2 != 3) {
        ...
        $sql3 = "SELECT A.*, B.DISKRIPSI FROM $schema1.padanan_peringkatakademik_skim A, $schema1.`ref_skim` B 
        WHERE A.kod_skim=B.KOD AND A.status=0 AND B.is_deleted=0 AND A.kod_peringkat_akademik=" . tosql($peringkat2);
    
        if (!empty($carian)) {
            $sql3 .= " AND B.DISKRIPSI LIKE '%$carian%'";
        }
    } else {
        $hrefs = 'index.php?data='.base64_encode('pengurusan/parameter/senarai_akademik_svm;Pentadbiran;Parameter;Padanan Peringkat Akademik Dan Skim;;;');
        echo $hrefs;
        //exit;
    }
                    ...
                    <tr>
                        ...
                        <td align="center">
                            <button type="button" class="btn btn-sm btn-danger" onclick="do_hapus('pengurusan/sql_pengurusan.php?frm=PARAMETER&jenis=PADANAN_PERINGKATAKADEMIK_SKIM&pro=HAPUS&peringkat2=<?=$peringkat2;?>&kod_skim=<?=$rsInstitusi->fields['kod_skim'];?>')">
                                <span style="cursor:pointer;color:red" title="Keluarkan maklumat jawatan"><i class="fa fa-trash-o" style="color: #FFFFFF;"></i></span>
                            </button>
                        </td>
                    </tr>
