...
<script>
	...
	function do_pengkhususan(pengkhususan) {
		// alert('sini');
		$.ajax({
			url:'pengurusan/sql_pengurusan.php?frm=PARAMETER&jenis=PENGKHUSUSAN&pro=CHECK_PENGKHUSUSAN&pengkhususan='+pengkhususan,
			type:'POST',
			//dataType: 'json',
			data: $("form").serialize(),
			success: function(data){
...
<div class="col-lg-12">
	<section class="panel">
		<header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
			...
			<h6 class="panel-title"><font color="#000000" size="3"><b><?php if(!empty($pengkhususan_kod)){ print 'KEMASKINI'; } else { print 'TAMBAH'; } ?> MAKLUMAT PENGKHUSUSAN</b></font></h6>
		</header>
		<div class="panel-body">
			<div class="box-body">

			<input type="hidden" name="kod" id="kod" value="<?php if(!empty($pengkhususan_kod)){ print $rs->fields['kod'];} ?>">
				...
				<div class="col-md-12">
                    <!-- <div class="form-group">
						<div class="row">
							...
							<div class="col-sm-10">
								<input type="text" name="kod" id="kod" class="form-control" value="<?php if(!empty($pengkhususan_kod)){ print $rs->fields['kod'];} ?>" onchange="do_kod(this.value)" <?php if(!empty($pengkhususan_kod)){ print 'readonly';} ?>>
							</div>
						</div>
					</div> -->
                    <div class="form-group">
						<div class="row">
							...
							<div class="col-sm-10">
								<textarea name="pengkhususan" id="pengkhususan" rows="2" class="form-control" onchange="do_pengkhususan(this.value)"><?php if(!empty($pengkhususan_kod)){ print $rs->fields['DISKRIPSI']; } ?></textarea>
							</div>
						</div>
					</div>
