...
$hrefs = 'index.php?data='.base64_encode('pengurusan/parameter/senarai_domainEmail;Pentadbiran;Parameter;Domain Emel;;;');
...
<div class="box" style="background-color:#F2F2F2">
    ...
    <header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
        ...
        <h6 class="panel-title"><font color="#000000"><b>Senarai Maklumat Domain Emel</b></font></h6> 
    </header>
    </div>
    </div>            
    <br /> 
    <div class="col-md-12" style="background-color:#F2F2F2;">
        ...
        <label for="nama" class="col-sm-1 control-label">Status:</label>
            ...
            <div class="col-md-3" style="background-color:#F2F2F2">
                <input type="text" id="carian"  name="carian" value="<?=$carian;?>" class="form-control" placeholder="Domain emel">
            </div>
            <div class="col-md-2" style="background-color:#F2F2F2">
                <button type="button" class="btn btn-info" onclick="do_page('<?=$hrefs;?>')" title="Sila klik untuk carian maklumat Domain Emel">
                    <i class=" fa fa-search"></i> <font style="font-family:Verdana, Geneva, sans-serif">Cari</font>
                </button>
            </div>
	   <div class="col-md-2" style="background-color:#F2F2F2">

	        <a href="pengurusan/parameter/form_domainEmail.php?domainEmail_kod=" class="btn btn-md btn-primary" data-toggle="modal" data-target="#myModal" title="Tambah maklumat Domain Emel">
        	    <i class="fa fa-plus"></i> Tambah Domain Emel
        	</a>
            </div>
    ...
    <?php
        // $conn->debug=true;
        $sql = "SELECT * FROM $schema1.`ref_domain_email` WHERE is_deleted=0"; 
        if($status2 != ''){
        ...
        // $rsdomainEmail = $conn->query($sql3);

        $sSQL1 = "SELECT COUNT(*) as total FROM $schema1.`ref_domain_email` WHERE is_deleted=0";
        if($status2 != ''){
    ...
    <div class="box-body" style="background-color:#F2F2F2">
        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
            <thead  style="background-color:rgb(38, 167, 228)">
                ...
                <th width="40%"><font color="#000000"><div align="center">Domain Emel</div></font></th>
            ...
            <?php 
                ...
                <tr>
                    ...
                    <td align="center">
                        <a href="pengurusan/parameter/form_domainEmail.php?domainEmail_kod=<?=$rs->fields['kod'];?>" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal" title="Kemaskini maklumat Domain Emel">
                            <i class="fa fa-edit"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-danger" onclick="do_hapus('pengurusan/sql_pengurusan.php?frm=PARAMETER&jenis=DOMAIN_EMAIL&pro=HAPUS&kod=<?=$rs->fields['kod'];?>')" title="Hapus maklumat Domain Emel">
