...
<script>
	...
	var negara_universiti = $('#negara_universiti').val();
	var jenis_institusi = $('#jenis_institusi').val();
	var status_SAH_YT = $('#status_SAH_YT').val();
    ...
    if(kod.trim() == '' ){
        ...
        alert_msg('Sila isi maklumat universiti.');
        $('#universiti').focus(); return true;
    } else if(negara_universiti.trim() == '' ){
        alert_msg('Sila pilih maklumat negara universiti.');
        $('#negara_universiti').focus(); return true;
    } else if(jenis_institusi.trim() == '' ){
        alert_msg('Sila pilih maklumat jenis institusi.');
        $('#jenis_institusi').focus(); return true;
    } else if(status_universiti.trim() == '' ){
        alert_msg('Sila pilih maklumat status universiti.');
        $('#status_universiti').focus(); return true;
    } else if(status_SAH_YT.trim() == '' ){
        alert_msg('Sila pilih maklumat status SAH data universiti.');
        $('#status_SAH_YT').focus(); return true;
    } else { 
		$.ajax({
...
<div class="col-lg-12">
	<section class="panel">
		<header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
			...
			<h6 class="panel-title"><font color="#000000" size="3"><b><?php if(!empty($universiti_kod)){ print 'KEMASKINI'; } else { print 'TAMBAH'; } ?> MAKLUMAT UNIVERSITI</b></font></h6>
		</header>
		<div class="panel-body">
			<div class="box-body">
				...
				<div class="col-md-12">
					...
					<div class="form-group">
						<div class="row">
                            <label for="tajuk" class="col-sm-2 control-label"><b>Status SAH<font color="#FF0000">*</font> : </b></label>
							<div class="col-sm-3">
                                <select name="status_SAH_YT" id="status_SAH_YT" class="form-control">
                                    <option value="">Sila pilih status</option>
                                    <option value="Y" <?php if(!empty($universiti_kod)){ if($rs->fields['SAH_YT'] == 'Y'){ print 'selected';} } ?>>SAH</option>
                                    <option value="T" <?php if(!empty($universiti_kod)){ if($rs->fields['SAH_YT'] == 'T'){ print 'selected';} } ?>>Tidak SAH</option>
                                </select>
                            </div>
						</div>
					</div>
