...
<?php
    ...
    $sql3 = "SELECT A.*, B.DISKRIPSI FROM $schema1.padanan_institusi_pengkhususan A, $schema1.`ref_pengkhususan` B WHERE A.kod_pengkhususan=B.kod AND A.status=0 AND B.`STATUS`=0 AND B.is_deleted=0 AND A.kod_institusi=".tosql($institusi_kod)." AND A.peringkat_kelulusan=".tosql($peringkatKelulusan);
