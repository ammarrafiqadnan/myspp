...
$hrefs = 'index.php?data='.base64_encode('pengurusan/kod_bidang/senarai_kod_bidang;Padanan Kluster;Maklumat Bidang;ALL;;;');
...
<div class="box" style="background-color:#F2F2F2">
    ...
    <header class="panel-heading"  style="background-color:rgb(38, 167, 228)">
        ...
        <h6 class="panel-title"><font color="#000000"><b>Senarai Maklumat Bidang</b></font></h6> 
    </header>
    </div>
    </div>            
    <br /> 
    <div class="col-md-12" style="background-color:#F2F2F2;">
        <div class="row">
            <?php $rsMP = $conn->query("SELECT * FROM $schema1.`ref_kluster` WHERE is_deleted=0 AND status=0 ORDER BY diskripsi ASC"); ?>
    ...
    <div class="col-md-12" style="background-color:#F2F2F2;">
        <div class="row">
            ...
            <div class="col-md-5" style="background-color:#F2F2F2">
                <input type="text" id="carian"  name="carian" value="<?=$carian;?>" class="form-control" placeholder="Kod/Bidang">
            </div>
            <div class="col-md-2" style="background-color:#F2F2F2">
                <button type="button" class="btn btn-info" onclick="do_page('<?=$hrefs;?>')" title="Sila klik untuk membuat carian maklumat Matapelajaran dan Bidang">
                    <i class=" fa fa-search"></i> <font style="font-family:Verdana, Geneva, sans-serif">Cari</font>
                </button>
            </div>

            <div class="col-md-2" style="background-color:#F2F2F2">
                <a href="pengurusan/kod_bidang/form_kod_bidang.php?id=" class="btn btn-md btn-primary" data-toggle="modal" data-target="#myModal" title="Tambah maklumat Matapelajaran dan Bidang">
    ...
    <?php
        //$conn->debug=true;
        $sql = "SELECT * FROM $schema1.`ref_bidang` WHERE is_deleted=0"; 
        if($status2 != ''){
            $sql .= " AND `status`=".$status2;
        }

        if($matapelajaran!= ''){
            $sql .= " AND `kod_mpelajaran`=".$matapelajaran;
        }
        ...
        $sSQL1 = "SELECT COUNT(*) as total FROM $schema1.`ref_bidang` WHERE is_deleted=0";
    ...
    <div class="box-body" style="background-color:#F2F2F2">
        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
            <thead  style="background-color:rgb(38, 167, 228)">
                ...
                <th width="20%"><font color="#000000"><div align="center">Matapelajaran</div></font></th>
                <th width="30%"><font color="#000000"><div align="center">Bidang</div></font></th>
                <!-- <th width="40%"><font color="#000000"><div align="center">Tahap Pendidikan</div></font></th> -->
			...
			$mp_desc = dlookup("$schema1.`ref_kluster`","diskripsi","kod=".tosql($rs->fields['kod_mpelajaran']));
		?>
                <tr>
                    <td align="center"><?=++$bil;?></td>
                    <td align="center"><?=$mp_desc;?></td>
                    ...
                    <td align="center">
			<?php //$conn->debug=true;
			$rsc = $conn->query("SELECT kod_bidang FROM $schema1.ref_padanan_kluster WHERE kod_bidang=".tosql($rs->fields['kod']));
			$cntb = $rsc->recordcount();
			?>
                        <a href="pengurusan/kod_bidang/form_kod_bidang.php?bidang_kod=<?=$rs->fields['kod'];?>" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal" title="Kemaskini maklumat Matapelajaran dan Bidang">
                            <i class="fa fa-edit"></i>
                        </a>
                        <button type="button" class="btn btn-sm btn-danger" title="Hapus maklumat Matapelajaran dan Bidang" 
			<?php if($cntb==0){ ?> onclick="do_hapus('pengurusan/sql_pengurusan.php?frm=PARAMETER&jenis=BIDANG&pro=HAPUS&kod=<?=$rs->fields['kod'];?>')"
			<?php } else { ?>  onclick="alert_msg('Maklumat Bidang ini tidak boleh dihapuskan kerana terdapat maklumat sedang digunakan')"
			<?php } ?>
			>
