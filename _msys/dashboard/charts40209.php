...
// print $start_dt;
$sql = "SELECT COUNT(*) as nilai FROM $schema2.calon_jawatan_dipohon A, $schema2.calon B  WHERE A.id_pemohon=B.id_pemohon AND B.pengakuan='Y' AND A.kod_peringkat IN('7','8','9','10')";
...
$sql = "SELECT COUNT(*) as nilai FROM $schema2.calon_jawatan_dipohon A, $schema2.calon B  WHERE A.id_pemohon=B.id_pemohon AND B.pengakuan='Y' AND A.kod_peringkat IN('5')";
...
$sql2 = "SELECT COUNT(*) as nilai FROM $schema2.calon_jawatan_dipohon A, $schema2.calon B  WHERE A.id_pemohon=B.id_pemohon AND B.pengakuan='Y' AND A.kod_peringkat IN('1')";
if(!empty($start_dt)){
  if(!empty($end_dt)){
...
$AKP11 = $conn->query($sql2);

$sql2 = "SELECT COUNT(*) as nilai FROM $schema2.calon_jawatan_dipohon A, $schema2.calon B  WHERE A.id_pemohon=B.id_pemohon AND B.pengakuan='Y' AND A.kod_peringkat IN('2','3')";
if(!empty($start_dt)){
  if(!empty($end_dt)){
    $sql2 .= " AND ((date(A.d_cipta) BETWEEN ".tosql($start_dt)." AND ".tosql($end_dt).") OR date(A.d_kemaskini) BETWEEN ".tosql($start_dt)." AND ".tosql($end_dt).")";
  } else {
...
<script type="text/javascript">
  $(function () {
      ...
      datasets: [
        {
          data: ['<?=$DH29->fields['nilai'];?>','<?=$DH41->fields['nilai'];?>','<?=$AKP11->fields['nilai'];?>','<?=$AKP19->fields['nilai'];?>'],
          backgroundColor : ['#f39c12', '#00c0ef', '#3c8dbc', '#009'],
        }
      ]
    }
...
<b>Nota: Jumlah Permohonan</b><br>
  <?php
  $jum = $DH29->fields['nilai']+$DH41->fields['nilai']+$AKP11->fields['nilai']+$AKP19->fields['nilai'];
