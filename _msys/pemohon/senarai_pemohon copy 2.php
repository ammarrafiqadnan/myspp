...
<?php 
    ...
    // $conn->debug=true;
    $sql = "SELECT A.id_pemohon,A.nama_penuh, A.ICNo,A.d_cipta,A.d_kemaskini,A.pengakuan, B.diskripsi2 FROM $schema2.calon A, $schema2.ref_negeri B WHERE A.negeri=B.kod AND A.d_kemaskini IS NOT NULL AND A.d_kemaskini >=".tosql($dby);
    ...
    // $strSQL=$sql. " LIMIT 10";
    // $rs = $conn->query($sql);
    $sSQL1 = "SELECT COUNT(*) as total FROM $schema2.calon A, $schema2.ref_negeri B WHERE A.negeri=B.kod AND A.d_kemaskini IS NOT NULL AND A.d_kemaskini >=".tosql($dby);
