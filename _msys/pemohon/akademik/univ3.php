<small style="color: red;">
    Tarikh dikemaskini : 
    <?php
    if(!empty($rsUniv3->fields['d_kemaskini'])){
        print DisplayDate($rsUniv3->fields['d_kemaskini']);  print '&nbsp;&nbsp;'.DisplayMasa($rsUniv3->fields['d_kemaskini']);
    } else {
        print DisplayDate($rsUniv3->fields['d_cipta']);  print '&nbsp;&nbsp;'.DisplayMasa($rsUniv3->fields['d_cipta']);
    }
     ?>  <br>
...
<div class="form-group">
	<div class="row">
		<div class="col-sm-8">
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-3">
						<?php print $rsUniv3->fields['tahun']; ?>	
					</div>
				</div>
			</div>
				...
				<div class="row">
					...
					<div class="col-sm-8">
                        <?php while(!$rfKelulusan->EOF){ ?>
                            <?php if($rsUniv3->fields['peringkat']==$rfKelulusan->fields['kod']){ 
				...
				<div class="row">
					...
					<div class="col-sm-8">
                        <?php while(!$rsInstitusi->EOF){ ?>
                            <?php if($rsUniv3->fields['inst_keluar_sijil']==$rsInstitusi->fields['KOD']){ 
				...
				<div class="row">
					...
					<div class="col-sm-8">
							<?php while(!$rsKhusus->EOF){ ?>
							    <?php if($rsUniv3->fields['pengkhususan']==$rsKhusus->fields['kod']) { 
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-8">
                            ...
                            <?php if( $rfMajor->fields['KOD'] == $rsUniv3->fields['MAJOR'] ){ 
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-8">
                            ...
                            <?php if( $rfMinor->fields['KOD'] == $rsUniv3->fields['MINOR'] ){ 
				...
				<div class="row">
					...
					<div class="col-sm-8">
        					<?php while(!$rsJP->EOF){ $code = $rsJP->fields['kod']; ?>    
            					<?php if($rsUniv3->fields['muet']==$code){ print $rsJP->fields['diskripsi']; }?>
        					<?php $rsJP->movenext(); } ?>
				...
				<div class="row">
					...
					<div class="col-sm-4">
						...
						<?php for($i=$d; $i>='1999'; $i--){ ?>
                   					<?php if($rsUniv3->fields['muet_tahun']==$i){ print $i; }?>                    				<?php } ?>					
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-sm-7">
							...
							?>    
            						<?php if($rsUniv3->fields['muet_gred']==$code){ print $kep; }?>
        					<?php $rsJK->movenext(); } ?>
					</div>
				...
				<div class="row">
					...
					<div class="col-sm-8">
                        <?php while(!$rsBiasiswa->EOF){ ?>
                            <?php if($rsUniv3->fields['biasiswa']==$rsBiasiswa->fields['kod_biasiswa']){ 
		...
		$rsSijil3 = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='UNIV3B' AND `id_pemohon`=".tosql($uid));
		//if(empty($rsSijil->fields['sijil_nama'])){ $sijil2 ="../upload_doc/sijil_muet.jpg"; }
		//else { $sijil2 = "/upload/".$uid."/".$rsSijil->fields['sijil_nama']; }
		if(empty($rsSijil3->fields['sijil_nama'])){ $sijil_pic2 ="/var/www/html/myspp/upload_doc/sijil_muet.jpg"; }
		...
		$rsSijil31 = $conn->query("SELECT * FROM $schema2.`calon_sijil` WHERE `jenis_sijil`='UNIV3C' AND `id_pemohon`=".tosql($uid));
		?>
		...
		<div class="col-sm-4" align="center">
			...
			<?php if(empty($rsSijil31->fields['sijil_nama'])){ 
				...
				?>
				<!--<a href="../view_doc_pemohon.php?id_pemohon=<?=$uid;?>&doc=<?=$rsSijil31->fields['sijil_nama'];?>" data-toggle="modal" data-target="#myModal"><?=$rsSijil31->fields['sijil_nama'];?></a> -->
		...
		<div class="col-sm-4" align="center">
			...
			<?php if (file_exists($sijil_pic2)){ print "<br><div>".$rsSijil3->fields['sijil_nama']."</div>"; } ?>
		</div>
