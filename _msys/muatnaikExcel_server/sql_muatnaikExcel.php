		...
		if(!empty($file_name)){
			...
			if (($handle = fopen($filename, "r")) !== FALSE) {
				...
				while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
					...
					$nokp 	  	= trim($data[2]);		// B
					$skim 		= trim($data[3]);	// C
					$tarikh 	= trim($data[4]);	// D
					$masa   	= date('h:i:s', strtotime(trim($data[5])));	// E
					$tempat   	= trim($data[6]);		// F
					...
					if(!empty($nokp) && $row>=2){
						...
						//exit;
						if(!$rs->EOF){ 	
							$sql = "UPDATE $schema2.`senarai_panggilan_temuduga` SET nama_penuh=".tosql($nama).", noKP=".tosql($nokp).", skim_jawatan=".tosql($skim).", tarikh=".tosql(date('Y-m-d',strtotime($tarikh))).", masa=".tosql($masa).", tempat=".tosql($tempat).", no_pemerolehan=".tosql($no_pemerolehan); 	
