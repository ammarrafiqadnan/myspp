...
if(!empty($file_name) && $pro=='PROSES'){
	//$upload_dir = 'zip/';
	$upload_dir = '/var/www/html/myspp/_msys/zip/';
	...
	if ($_FILES["file1"]["error"] > 0){
		...
		if(move_uploaded_file($_FILES["file1"]["tmp_name"], $nama_fails)) { 
			//print "The file ". basename($uploadedfile). " has been uploaded"; 
		} else { 
			//print "<br>".$nama_fails."<br>Sorry, there was a problem uploading your file.";
	...
	//$datas = '';
	$row = 0; $bilr=0; $rows=0;
	if (($handle = fopen($upload_dir."data_spp.csv", "r")) !== FALSE) {
	...
	$filename = $upload_dir.'files.zip';
	// DELETE FILE IF EXIST
	if (file_exists($filename)) { unlink($filename); } //else { print 'HJOHOHO'; }
