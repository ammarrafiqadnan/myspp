...
if($frm=='PANGGILAN_TEMUDUGA'){
	...
	if($pro=='SAVE'){
		...
		//print "DOC:".$_FILES['dokumen']['name'];
		$final_image = '';

			if(!empty($_FILES['dokumen']['name'])){
				$valid_extensions = array('pdf', 'PDF'); // valid extensions
				$path = '/var/www/html/myspp/upload_doc/'; // upload directory
				
				$img = $_FILES['dokumen']['name'];
				$tmp = $_FILES['dokumen']['tmp_name'];
				
				$ext = end((explode(".", $img))); 
				$fname = str_replace(" ", "_", $img);
				$fname = str_replace("-", "_", $fname);
				$final_image = strtolower($fname);
		...
		if(!empty($kod)){
            // print 'sini'; exit();
			$strSQL = "UPDATE $schema2.`keputusan_temuduga` SET tajuk=".tosql($tajuk).", kod_surat=".tosql($kod_surat).", file_lampiran=".tosqln($final_image).", 
			tarikh_mula=".tosql($tarikh_mula).", masa_mula=".tosql($masa_mula).", tarikh_tamat=".tosql($tarikh_tamat).", masa_tamat=".tosql($masa_tamat).", 
            ...
            // print 'sini2'; exit();
			$sql = "INSERT INTO $schema2.`keputusan_temuduga` (tajuk, kod_surat, file_lampiran, tarikh_mula, masa_mula, tarikh_tamat, masa_tamat, tarikh_cipta, id_pencipta) ";
			$sql .= " VALUES(".tosql($tajuk).",".tosql($kod_surat).",".tosqln($final_image).",".tosql($tarikh_mula).",".tosql($masa_mula).",".tosql($tarikh_tamat).",".tosql($masa_tamat).",".tosql($date).",".tosql($oleh).")";
			$rss = $conn->Execute($sql);
		}
		...
		if(!empty($file_name)){
			...
			if ($_FILES["file1"]["error"] > 0){ 
				...
				// print $_FILES["file1"]["tmp_name"];
				$nama_fails =  $upload_dir.$file_name;
				if(move_uploaded_file($_FILES["file1"]["tmp_name"], $nama_fails)) { //print 'sini';
					$text = "The file ". basename($nama_fails). " has been uploaded"; 
				} else { //print 'sini2';
					$text .= "<br>".$nama_fails."<br>Sorry, there was a problem uploading your file.";
					//$text .= "<br>".phpFileUploadErrors($_FILES['file1']['error']); 
			...
			if (($handle = fopen($filename, "r")) !== FALSE) {
				...
				while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
					...
					if(!empty($nokp) && $row>=2){
						...
						//exit;
						if(!$rs->EOF){ 	
							$sql = "UPDATE $schema2.`senarai_keputusan_temuduga` SET nama_penuh=".tosql($nama).", noKP=".tosql($nokp).", kod_skim=".tosql($kod_skim).", skim_jawatan=".tosql($skim).", tarikh=".tosql(date('Y-m-d',strtotime($tarikh))).", tempat=".tosql($tempat).", no_pemerolehan=".tosql($no_pemerolehan). ", `status`=".tosql($status).", keputusan1=".tosql($keputusan1).", keputusan2=".tosql($keputusan2); 	
							$sql .= " WHERE kod=".tosql($rs->fields['kod']);
						} else {
							$sql = "INSERT INTO $schema2.`senarai_keputusan_temuduga` (no_pemerolehan,kod_keputusan_temuduga,nama_penuh, noKP, kod_skim, skim_jawatan, tarikh, tempat, `status`, keputusan1, keputusan2) ";
							$sql .= " VALUES(".tosql($no_pemerolehan).",".tosql($kod_keputusan_temuduga).",".tosql($nama).",".tosql($nokp).",".tosql($kod_skim).",".tosql($skim).",".tosql(date('Y-m-d',strtotime($tarikh))).",".tosql($tempat).",".tosql($status).",".tosql($keputusan1).",".tosql($keputusan2).")";
						}
						$rss = $conn->execute($sql);
						//$err='OK';
					}
		...
		$sql2 = "DELETE FROM `senarai_keputusan_temuduga` WHERE kod_keputusan_temuduga='{$kod}'";
		$rss2 = $conn->Execute($sql2);
	...
	if($pro=='SAVE'){
		...
		$sql2 = "DELETE FROM `senarai_rayuan_temuduga` WHERE kod_rayuan_temuduga='{$kod}'";
		$rss2 = $conn->Execute($sql2);
...
} else if ($frm=='PENETAPAN_TARIKH') {
	if($pro == 'SAVE'){
		$kod=isset($_REQUEST["kod"])?$_REQUEST["kod"]:"";
		$kod_pk=isset($_REQUEST["kod_pk"])?$_REQUEST["kod_pk"]:"";
		...
		if($kategori == 'panggilan'){
			$sql = "UPDATE $schema2.`senarai_panggilan_temuduga` SET tarikh_mula=".tosql($tarikh_mula).",tarikh_tamat=".tosql($tarikh_tamat).", 
			masa_mula=".tosql($masa_mula).",masa_tamat=".tosql($masa_tamat)." WHERE kod=".tosql($kod)." AND kod_panggilan_temuduga=".tosql($kod_pk);
			$rss = $conn->Execute($sql);
		} else if($kategori == 'keputusan'){
			$sql = "UPDATE $schema2.`senarai_keputusan_temuduga` SET tarikh_mula=".tosql($tarikh_mula).",tarikh_tamat=".tosql($tarikh_tamat).", 
			masa_mula=".tosql($masa_mula).",masa_tamat=".tosql($masa_tamat)." WHERE kod=".tosql($kod)." AND kod_keputusan_temuduga=".tosql($kod_pk);
			$rss = $conn->Execute($sql);
		}
