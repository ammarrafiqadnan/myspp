...
$hrefs = 'index.php?data='.base64_encode('muatnaikExcel/senarai_pemohon_panggilan_temuduga;PENGURUSAN;Senarai Panggilan Temu Duga;;;1;');
		...
		<div class="box" style="background-color:#F2F2F2">
            ...
            <div class="col-md-12" style="background-color:#F2F2F2;">
                <div class="row">
                    ...
                    <div class="col-md-1">
                        <a href="" class="btn btn-md btn-success" data-toggle="tooltip" data-title="Salin Maklumat Kepada excel/csv" onclick="do_print('cetak.php?pages=muatnaikExcel/panggilan_temuduga_cetak&prn=EXCEL&filename=senarai_panggilan_temuduga&kod=<?=$id;?>')" >
                            <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                        </a>
                    </div>
                    <!--<div class="col-md-2" align="left">
                        <button class="btn btn-md btn-info" title="Muat Turun Dokumen" style="width:100%"><i class="fa fa-download"></i>  Muat Turun Dokumen</button>
                    </div>-->
            ...
            <?php
		//$conn->debug=true;
                $sql3 = "SELECT * FROM $schema2.`panggilan_temuduga` WHERE kod='{$id}'";
                $rsPT = $conn->query($sql3);

                //$conn->debug=true;
                $sql = "SELECT * FROM $schema2.`senarai_panggilan_temuduga` WHERE kod IS NOT NULL AND is_deleted=0 AND kod_panggilan_temuduga='{$id}'"; 
                if(!empty($carian)){
                ...
                $sSQL1 = "SELECT COUNT(*) as total FROM $schema2.`senarai_panggilan_temuduga` WHERE kod IS NOT NULL AND is_deleted=0 AND kod_panggilan_temuduga='{$id}'";
                if(!empty($carian)){
			...
			//$conn->debug=true;
                        ...
                        while(!$rs->EOF){ $bil2=0; 
                            ...
                            $id_pemohon = dlookup($schema2.'.calon','id_pemohon','ICNo='.tosql($rs->fields['noKP']));
			    $href_cetakB = "modal_print.php?win=".base64_encode('cetakan/cetak_permohonan.php;'.$id_pemohon.";".$rs->fields['noKP']);
				...
				<?php } ?>
                                ...
                                <a href="" class="btn btn-sm btn-primary" onclick="do_cetak2('<?=$href_cetakB;?>')" titla="Slip">
                                    <i class="fa fa-certificate" aria-hidden="true"></i>
                                </a>

				<?php if($_SESSION['SESSADM_UROLE'] != 3){ ?>
                                <button type="button" class="btn btn-sm btn-danger" onclick="do_hapus('muatnaikExcel/sql_muatnaikExcel.php?frm=PEMOHON_PANGGILAN_TEMUDUGA&pro=HAPUS&kod=<?=$rs->fields['kod'];?>')">
                ...
                <a href="index.php?data=<?php print base64_encode('muatnaikExcel/senarai_panggilan_temuduga;Pengurusan;Senarai Panggilan Temu Duga;;;;'); ?>" class="btn btn-md btn-success" title="Senarai Panggilan Temuduga">
