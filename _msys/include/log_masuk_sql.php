...
if(!empty($pengguna) && $pro=='CHK'){
	...
	if(($rsT->fields['total']) >= 1){
		if(!$rs->EOF){
			...
			// if($user_katamasuk == $pass){
				$_SESSION['SESSADM_USER']="ADMINMYSPP";
				$_SESSION['SESSADM_UID']=$rs->fields['id'];
				$_SESSION['SESSADM_ULOG']=$rs->fields['username'];
				// $_SESSION['SESSADM_UIC']=$rs->fields['MyID'];
				$_SESSION['SESSADM_UNAME']=$rs->fields['nama_penuh'];
				$_SESSION['SESSADM_UROLE']=$rs->fields['peranan'];	
				$_SESSION['SESSADM_MENU']=$rs->fields['menu'];	
				$err = 'OK';
			// } else {
			// 	$err = 'ERR';
			// }
			// print $_SESSION['SESSADM_UROLE'].'/'.$_SESSION['SESSADM_UNAME'].'/'.$_SESSION['SESSADM_UID'];
		}
	} else {
		$err='XADA';
	}
	...
	if($rs->recordcount()>=1){
		if(!$rs->EOF){
			...
			if($user_katamasuk == $pass){
				$_SESSION['SESSADM_UID']=$rs->fields['id_pemohon'];
				$_SESSION['SESSADM_UIC']=$rs->fields['MyID'];
				$_SESSION['SESSADM_UNAME']=$rs->fields['nama'];			
				//print $_SESSION['SESSADM_UID'];
				$err = 'OK';
			} else {
...
//print $_SESSION['SESSADM_UID']; exit;
//if(!empty($pengguna) && $pro=='CHK'){
