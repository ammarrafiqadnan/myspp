...
for($i=0;$i<=12;$i++){ 
    ...
    $kategori = $bulan;
    $jumlah = dlookup("tbl_isuakidah","count(*)","is_deleted=0 AND year(tarikh)='{$tahun_list}' AND month(tarikh)='{$i}'");
    ...
    $sql = "SELECT B.`topik_nama`, count(*) as nilai FROM `tbl_isuakidah` A, `_ref_topik` B 
    WHERE A.topik_id=B.topik_id AND A.`is_deleted`=0 AND year(A.tarikh)='{$tahun_list}' AND month(A.tarikh)='{$i}' GROUP BY B.topik_id";
    // print $sql."<br>";
    ...
    if(!$rsData->EOF){ 
        $datas .= '{
                ...
                data: [';
        while(!$rsData->EOF){
            $datas .= '["'.$rsData->fields['topik_nama'].'", '.$rsData->fields['nilai'].'],';
            $rsData->movenext();
        }
        $datas .= '] },';
    }
...
<div class="box-body">
    <b>LAPORAN ISU-ISU AKIDAH DI MEDIA BARU BAGI TAHUN</b> 
    <div class="row">
        <div class="col-md-3">
...
Highcharts.chart('akhbar_tahun', {
    ...
    title: {
        align: 'left',
        text: 'LAPORAN ISU-ISU AKIDAH DI MEDIA BARU BAGI TAHUN <?=$tahun_list;?>'
    },
    // subtitle: {
    //     align: 'left',
