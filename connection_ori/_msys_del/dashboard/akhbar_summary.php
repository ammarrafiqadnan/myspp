...
$sql = "SELECT * FROM `_ref_isuagama` WHERE `is_deleted`=0 AND `isuagama_status`=0 ORDER BY `isuagama_nama`";
$rs = $conn->query($sql);
$cols = $rs->recordcount();
$jcols=$cols+2;
...
function get_data($tahun,$bulan,$kategori){
	global $conn;
	$sqls = "SELECT count(*) AS jum FROM `tbl_isu_agama` WHERE is_deleted=0 AND `kategori_id`='{$kategori}' AND year(tarikh)='{$tahun}'";
...
?>    
<div align="center">
STATISTIK PEMANTAUAN ISU-ISU SEMASA AGAMA DI AKHBAR TAHUN <?=$tahun_list;?>
</div>
<div class="box-body" style="background-color:#F2F2F2">
  	<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
        ...
        <tr>
          ...
          <?php while(!$rs->EOF){ ?>
          <th width="8%"><font color="#000000"><div align="center"><?=$rs->fields['isuagama_nama'];?></div></font></th>
          <?php $rs->movenext(); } ?>
