...
function admin_audit_trail($remarks, $action){
  global $conn, $schema2;
  // global $schema2;
  $sSQL = "";
  $page_name = $_SESSION['page_name']; //curPageName();
  $idusers = $_SESSION["SESSADM_UID"];
  $users = $_SESSION["SESSADM_ULOG"];
  $ip = $_SERVER['REMOTE_ADDR'];
...
function set_tarikh($id_pemohon, $fields){
    global $conn, $schema2;
    $dtk = date("Y-m-d H:i:s");
    $rs = $conn->query("SELECT * FROM $schema2.`calon_tarikh` WHERE `id_pemohon`=".tosql($id_pemohon));
    if($rs->EOF){
        $conn->execute("INSERT INTO $schema2.`calon_tarikh`(`id_pemohon`) VALUES(".$id_pemohon.")");
    }

    $sql = "UPDATE $schema2.`calon_tarikh` SET $fields=".tosql($dtk).", `pengakuan`=NULL, `tarikh_akuan`=NULL, `tkh_upd_perakuan`=NULL 
      WHERE `id_pemohon`=".tosql($id_pemohon);
    $conn->execute($sql);

    $sql1 = "UPDATE $schema2.`calon` SET `d_kemaskini`=".tosql($dtk).", `pengakuan`=NULL, `tarikh_akuan`=NULL WHERE `id_pemohon`=".tosql($id_pemohon);
    $conn->execute($sql1);
...
$user_id=isset($_SESSION['SESS_UID'])?$_SESSION['SESS_UID']:"";
$userid=isset($_SESSION['SESS_UID'])?$_SESSION['SESS_UID']:"";
$update_by=isset($_SESSION['SESS_UID'])?$_SESSION['SESS_UID']:"";
